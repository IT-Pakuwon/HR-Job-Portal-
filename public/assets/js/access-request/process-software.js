async function openProcessSoftwareModal(id) {
    ...
    try {
        ...
        const details = (res.details ?? []).filter(item => item.group_category === 'SOFTWARE');
...
function renderProcessSoftwareError(xhr) {
    const message = xhr.responseJSON?.message ?? 'Failed load software process';
    const html = `
...
function renderProcessSoftwareItems(access, details = []) {
    ...
    details.forEach(item => {
        ...
        const pwField = isCompleted
            ...
            : `<div class="relative">
                   <input type="password"
                       class="software-password w-full rounded-lg border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-[#0b1525] px-4 py-3 pr-10 text-sm text-slate-700 dark:text-slate-100 placeholder:text-slate-400 focus:border-blue-500/30 focus:ring-0"
                       data-id="${item.id}"
            ...
            <tr class="border-b border-slate-100 dark:border-white/[0.06] align-top">
                ...
                <td class="px-5 py-5 min-w-[280px]">
                    ${isCompleted
                        ...
                        : `<textarea rows="3" class="software-response w-full rounded-lg border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-[#0b1525] px-4 py-3 text-sm text-slate-700 dark:text-slate-100 placeholder:text-slate-400 focus:border-blue-500/30 focus:ring-0" data-id="${item.id}" placeholder="Input response ...">${item.access_response ?? ''}</textarea>`}
                </td>
                <td class="px-5 py-5 min-w-[220px]">
                    ${isCompleted
                        ...
                        : `<input type="text" class="software-username w-full rounded-lg border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-[#0b1525] px-4 py-3 text-sm text-slate-700 dark:text-slate-100 placeholder:text-slate-400 focus:border-blue-500/30 focus:ring-0" data-id="${item.id}" value="${item.access_username ?? ''}" placeholder="Username">`}
                </td>
                ...
                <td class="px-5 py-5 min-w-[180px]">
                    ${isCompleted
                        ...
                        : `<div class="flex flex-col gap-2">
                               <button type="button" class="btn-save-software inline-flex h-10 w-full items-center justify-center rounded-lg border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-[#0b1525] px-4 text-xs font-semibold text-slate-700 dark:text-slate-200 transition hover:bg-slate-100 dark:hover:bg-white/[0.06]"
                                   data-doc="${access.eid}" data-id="${item.id}" data-type="${access.access_type ?? ''}">
                                   <i class="fa-solid fa-floppy-disk mr-2 text-[11px]"></i>Save
                               </button>
                               <button type="button" class="btn-complete-software inline-flex h-10 w-full items-center justify-center rounded-lg bg-slate-900 dark:bg-white px-4 text-xs font-semibold text-white dark:text-slate-900 transition hover:bg-slate-800 dark:hover:bg-slate-200"
                                   data-doc="${access.eid}" data-id="${item.id}" data-type="${access.access_type ?? ''}">
    ...
    if (!rows) {
        rows = `<tr><td colspan="6" class="px-5 py-12 text-center text-sm text-slate-500 dark:text-slate-400">No software request available</td></tr>`;
    }

    $('#processSoftwareDetailContainer').html(`
        <div class="overflow-hidden rounded-lg border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-[#111c2d]">
            <div class="border-b border-slate-200 dark:border-white/[0.06] px-5 py-4">
                <h3 class="text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">Software Request Detail</h3>
            </div>
...
// Eye toggle for password inputs in process modals
$(document).on('click', '.btn-toggle-pw-input', function () {
    const btn     = $(this);
...
$(document).on('click', '.btn-save-software', async function () {
    ...
    const response = $(`.software-response[data-id="${detailId}"]`).val()?.trim();
    const username = $(`.software-username[data-id="${detailId}"]`).val()?.trim();
    const password = $(`.software-password[data-id="${detailId}"]`).val()?.trim();

    await submitSoftwareItem(button, hash, detailId, response, username, password, 'SAVE');
});
...
$(document).on('click', '.btn-complete-software', async function () {
    ...
    const hash       = button.data('doc');
    const detailId   = button.data('id');
    const accessType = (button.data('type') ?? '').toUpperCase();
    const response   = $(`.software-response[data-id="${detailId}"]`).val()?.trim();
    const username   = $(`.software-username[data-id="${detailId}"]`).val()?.trim();
    const password   = $(`.software-password[data-id="${detailId}"]`).val()?.trim();

    if (accessType === 'NEW') {
        if (!username) { swalWarning('Username is required'); return; }
        if (!password) { swalWarning('Password is required'); return; }
    }

    await submitSoftwareItem(button, hash, detailId, response, username, password, 'DONE');
});

async function submitSoftwareItem(button, hash, detailId, response, username, password, action) {
    ...
    try {
        ...
        itrToast('success', res.message ?? 'Software access updated successfully');
        await openProcessSoftwareModal(hash);
        table.ajax.reload(null, false);

    } catch (xhr) {
        console.error(xhr);
        swalError(xhr.responseJSON?.message ?? 'Failed process software access');
    } finally {
