(function () {
    ...
    const urls = {
        summary: "/corporate-teknik-dashboard/summary-json",
        approval: "/corporate-teknik-dashboard/waiting-approval-json",
        approvalHistory: "/corporate-teknik-dashboard/approval-history-json",
        ticket: "/corporate-teknik-dashboard/ticket-json",
        beritaAcara: "/corporate-teknik-dashboard/berita-acara-json",
        doctypes: "/corporate-teknik-dashboard/approval-doctypes-json",
    };
    ...
    function renderSummary(data) {
        ...
        const stats = {
            ...
            openBeritaAcara: { count: data.open_berita_acara || 0 },
        };
    ...
    const rowConfig = {
        ...
        ticket: {
            ...
            link: row => `/showoprtekticket/${row.eid}`,
            subtitle: row => [
        ...
        "berita-acara": {
            ...
            title: row => row.ba_auto_number || row.ticketid,
            link: row => `/showoprtekticket/${row.eid}`,
            subtitle: row => [
                ticketTypeLabel(row.ticket_type),
                row.cpny_id,
                [row.location_name, row.sub_location_name].filter(Boolean).join(" / "),
                row.pic_ticket ? `PIC: ${row.pic_ticket}` : null,
            ].filter(Boolean).join(" • "),
            date: row => row.ticketdate,
            status: row => statusBadge((row.status_pekerjaan || "-").toUpperCase()),
            searchFields: row => [row.ticketid, row.ba_auto_number, row.issue_summary, row.user_peminta, row.cpny_id, row.pic_ticket],
        },
    };
