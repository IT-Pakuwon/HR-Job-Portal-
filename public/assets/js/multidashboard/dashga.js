(function () {
    ...
    function renderSummary(data) {
        ...
        const stats = {
            ...
            voucherTaxi:     { count: data.voucher_taxi || 0 },
            bookingCar:      { count: data.booking_car || 0 },
            freeParking:     { count: data.free_parking || 0 },
        };
    ...
    const tabConfig = {
        ...
        "voucher-taxi": {
            ...
            fields: row => [
                ...
                { label: "Owner", value: row.user_peminta_expense },
                { label: "From", value: row.origin },
                { label: "To", value: row.destination },
                { label: "Purpose", value: row.purpose_descr },
            ],
            searchFields: row => [row.docid, row.user_peminta, row.origin, row.destination, row.purpose_descr],
        },
        "booking-car": {
            ...
            fields: row => [
                ...
                { label: "Requester", value: row.user_peminta },
                { label: "Driver", value: row.driver },
                { label: "Purpose", value: row.purpose_descr },
            ],
            searchFields: row => [row.docid, row.user_peminta, row.driver, row.purpose_descr],
        },
        parking: {
            icon: "🅿️", badgeBg: "bg-violet-100 dark:bg-violet-900/30",
            title: row => row.docid,
            link: row => `${row.url}/${row.eid}`,
            status: row => statusBadge(row.status),
            fields: row => [
                { label: "Date", value: row.parking_regist_date },
                { label: "Requester", value: row.user_peminta },
                { label: "Type", value: row.parking_type },
                { label: "Worker", value: row.worker_type },
                { label: "Perpost", value: row.perpost },
            ],
            searchFields: row => [row.docid, row.user_peminta, row.parking_type, row.worker_type],
        },
    };
    ...
    const allTabs = ["approval", "approval-history", "voucher-taxi", "booking-car", "parking"];
