<x-authentication-layout>
    <div
        ...
        <div class="flex flex-row items-center justify-center gap-6">
            ...
            <h2 class="text-center text-lg font-extrabold text-gray-900 md:text-lg dark:text-gray-100">
                {{ __('APP SYSTEM') }}
            </h2>
        </div>

        <p class="mb-8 text-center text-sm text-gray-600 dark:text-gray-300">
        ...
        @if (session('status'))
            <div
                class="mb-4 rounded-md bg-green-50 p-3 text-xs font-medium text-green-600 dark:bg-green-900/20 dark:text-green-300">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="space-y-5">
                <div>
                    <x-label class="text-xs font-medium text-gray-700 dark:text-gray-200" for="login"
                ...
                <div>
                    <x-label class="text-xs font-medium text-gray-700 dark:text-gray-200" for="password"
                        value="{{ __('Password') }}" />
                    <x-input
                        class="mt-1 block w-full rounded-lg border border-gray-300 bg-gray-50 px-4 py-2 text-gray-900 shadow-sm transition-colors duration-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 dark:focus:border-indigo-400 dark:focus:ring-indigo-400"
            ...
            <div class="mt-6">
                <div class="flex items-center justify-end">
                    <a class="text-xs font-medium text-indigo-600 transition-colors duration-200 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300"
