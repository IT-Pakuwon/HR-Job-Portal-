    ...
    <div class="grid lg:grid-cols-[58%_42%] sm:h-full">
        ...
        <div class="relative hidden p-6 lg:block">
            ...
            <div
                ...
                {{-- Dark overlay so warp streaks read clearly over photos --}}
                <div class="absolute inset-0 bg-black/50"></div>

                {{-- Warp Speed Canvas (transparent bg — sits over the darkened photo) --}}
                <canvas id="warpCanvas" class="absolute inset-0 w-full h-full"></canvas>

                {{-- Radial glow from center --}}
                <div class="absolute inset-0 pointer-events-none" style="background: radial-gradient(ellipse at center, rgba(60, 81, 200, 0) 0%, rgba(0,0,0,0) 65%);"></div>
                ...
                <div class="absolute inset-x-0 top-0 z-20 flex items-center justify-between p-8">
                    ...
                    {{-- Clock --}}
                    <div class="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 backdrop-blur-xl shadow-lg shadow-black/10">
                        <div class="flex items-center gap-3">
                            <div class="flex items-center gap-2 text-white">
                                <span x-text="time" class="text-lg font-semibold tracking-tight"></span>
                                <span class="text-white/40">•</span>
                                <span x-text="date" class="text-md text-white/70"></span>
                            </div>
                        </div>
                    </div>
                ...
                {{-- Center Content --}}
                <div class="absolute inset-0 z-20 flex flex-col items-center justify-center text-white text-center px-10">

                    <h2 class="text-6xl font-bold leading-tight tracking-tight drop-shadow-lg">
                        APP SYSTEM
                    </h2>

                    <p class="mt-3 text-xl font-medium text-white/60 tracking-wide">
                        Pakuwon Group
                    </p>

                    {{-- Modules --}}
                    <div class="mt-8 flex flex-wrap justify-center gap-3 max-w-sm">

                        <span class="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-medium backdrop-blur-xl">
...
(function initWarp() {
    function start() {
        ...
        function makeStar(spread) {
            ...
            const dist  = spread ? Math.random() * Math.hypot(W, H) * 0.55 : Math.random() * 6;
            return {
                angle,
                dist,
        ...
        function reset(s) {
            ...
            s.speed = Math.random() * 1.8 + 0.4;
            s.size  = Math.random() * 1.4 + 0.3;
            s.blue  = Math.random() > 0.65;
        }

        resize();
        window.addEventListener('resize', () => { resize(); stars.forEach(s => reset(s)); });

        for (let i = 0; i < COUNT; i++) stars.push(makeStar(true));

        function frame() {
            // clear fully each frame — background image shows through canvas
            ctx.clearRect(0, 0, W, H);

            for (const s of stars) {
                s.dist += s.speed * (1 + s.dist * 0.006);

                const x1 = cx + Math.cos(s.angle) * s.dist;
                const y1 = cy + Math.sin(s.angle) * s.dist;

                if (x1 < -10 || x1 > W + 10 || y1 < -10 || y1 > H + 10) { reset(s); continue; }

                // Draw a long trail by reaching back behind the current position
                const trailLen = Math.max(4, s.speed * (1 + s.dist * 0.006) * 18);
                const tailDist = Math.max(0, s.dist - trailLen);
                const x0 = cx + Math.cos(s.angle) * tailDist;
                const y0 = cy + Math.sin(s.angle) * tailDist;

                const alpha = Math.min(0.9, s.dist / 80);
                const col   = s.blue ? `180,215,255` : `255,255,255`;
