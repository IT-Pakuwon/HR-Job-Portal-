                ...
                <div class="bg-white dark:bg-gray-700 p-6 rounded-lg w-1/3 relative">
                    ...
                    <form id="appForm">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="username" name="username" class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700" required>
                        </div>
                        ...
                        <div class="mb-4">
                            <label class="block text-gray-700 dark:text-white">Enter Password</label>
                            <input type="password" id="password" name="password" class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700" required>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="companyid[]" class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700 select2" multiple required>
                                @foreach($company as $p)
                                    <option value="{{ $p->Company_id }}">{{ $p->Company_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="position_id" class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700 select2" required>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="npk" class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700" required>
                        </div>
                        <div class="mb-4">
                            <label class="block text-gray-700 dark:text-white">Email</label>
                            <input type="email" name="email" class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700" required>
                        </div>
            ...
            $(document).ready(function () {
                let table = $('#usersTable').DataTable({
                    ...
                    columns: [
                        ...
                        { data: 'username',className: 'no-pointer' },                       
                        { data: 'name',className: 'no-pointer'  },  
                        { data: 'department', className: 'no-pointer' },
                        { data: 'position_id',className: 'no-pointer'  },                    
                        {
                ...
                $(document).on('click', '.editAppBtn', function () {
                    ...
                    $.get(`/eng/users/${appId}/edit`, function (app) {
                        $('#modalTitle').text("Edit User");
                        $('#id').val(app.id);
                        $('#name').val(app.name);
                        $('#username').val(app.username);           // ✅ username
                        $('#email').val(app.email);
                        $('input[name="npk"]').val(app.npk);        // ✅ pastikan name=“npk” pada input

                        $('select[name="position_id"]').val(app.position_id).trigger('change');
                ...
                $(document).on('change', '.toggleStatus', function () {
                    ...
                    $.ajax({
                        url: `/eng/users/${appId}/toggle-status`,
                        type: 'PUT',
                ...
                $('#appForm').submit(function (e) {
                        e.preventDefault();
                        let appId = $('#id').val();
                        let url = appId ? `/eng/users/${appId}` : "{{ route('userseng.store') }}";
