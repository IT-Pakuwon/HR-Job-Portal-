<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">
                Approval • Expired Product • Settlement
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-3 shadow-sm transition hover:shadow-md dark:border-slate-700 dark:bg-slate-800">
            ...
            <div class="mt-2 h-1 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
                <div id="waitingSettlementBar" class="h-full rounded-full bg-violet-500 transition-all duration-500" style="width:0%"></div>
            </div>
        </div>
...
<script>
    window.vplDashboardConfig = {
        typeLabel: 'Product',
        expiredTabLabel: 'Expired Product List',
        settlementTabLabel: 'Waiting For Settlement',
        routes: {
            summary: "{{ route('vpprmtn-dashboard.summary-json') }}",
            approval: "{{ route('vpprmtn-dashboard.waiting-approval-json') }}",
            approvalHistory: "{{ route('vpprmtn-dashboard.approval-history-json') }}",
            expired: "{{ route('vpprmtn-dashboard.expired-json') }}",
            waitingSettlement: "{{ route('vpprmtn-dashboard.waiting-settlement-json') }}",
            doctypes: "{{ route('vpprmtn-dashboard.approval-doctypes-json') }}",
        },
    };
</script>
