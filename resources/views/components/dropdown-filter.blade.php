...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    <div class="{{ $align === 'right' ? 'md:left-auto md:right-0' : 'md:left-0 md:right-auto' }} absolute left-0 right-auto top-full z-10 mt-1 min-w-56 origin-top-right overflow-hidden rounded-lg border border-gray-200 bg-white pt-1.5 dark:border-gray-700/60 dark:bg-gray-800"
        @click.outside="open = false" @keydown.escape.window="open = false" x-show="open"
        x-transition:enter="transition ease-out duration-200 transform"
        x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
        x-transition:leave="transition ease-out duration-200" x-transition:leave-start="opacity-100"
        x-transition:leave-end="opacity-0" x-cloak>
        <div class="px-3 pb-2 pt-1.5 text-xs font-semibold uppercase text-gray-400 dark:text-gray-500">Filters</div>
        <ul class="mb-4">
            <li class="px-3 py-1">
        ...
        <div class="border-t border-gray-200 bg-gray-50 px-3 py-2 dark:border-gray-700/60 dark:bg-gray-700/20">
            <ul class="flex items-center justify-between">
                <li>
                    <button
                        class="btn-xs border-gray-200 bg-white text-red-500 hover:border-gray-300 dark:border-gray-700/60 dark:bg-gray-800 dark:hover:border-gray-600">Clear</button>
                </li>
                <li>
                    <button
