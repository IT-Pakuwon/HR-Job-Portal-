            ...
            @if($searchable)
                <div class="relative">
                    ...
                    <input type="text" x-model="search" placeholder="{{ $searchPlaceholder }}"
                        class="w-48 rounded-lg border border-slate-200 bg-slate-50 py-1.5 pl-8 pr-3 text-xs text-slate-700 placeholder:text-slate-400 focus:border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-300 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-200 dark:placeholder:text-slate-500" />
                </div>
            @endif
        </div>
    </div>
                        ...
                        <tr class="border-t border-slate-100 dark:border-slate-700/60">
                            @foreach($normalizedCols as $col)
                                <th @if($sortable && !empty($col['key'])) @click="toggleSort({{ json_encode($col) }})" @endif
                                    class="whitespace-nowrap bg-slate-50 px-5 py-2.5 text-left text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:bg-slate-800/50 dark:text-slate-500 {{ ($sortable && !empty($col['key'])) ? 'select-none cursor-pointer hover:text-slate-600 dark:hover:text-slate-300 transition-colors' : '' }}">
                                    {{ $col['label'] }}
                                ...
                                @if(($col['type'] ?? null) === 'bar')
                                    <td class="px-5 py-3">
                                        <div class="flex items-center gap-2">
                                            <div class="h-1.5 w-full max-w-[140px] overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700/60">
                                            ...
                                            <span class="w-12 shrink-0 text-xs font-semibold text-slate-500 dark:text-slate-400" x-text="(row['{{ $col['key'] }}'] || 0) + '%'"></span>
                                        </div>
                                    </td>
                                @else
                                    <td class="px-5 py-3 text-slate-700 dark:text-slate-300" x-text="row['{{ $col['key'] }}']"></td>
                                @endif
                            @endforeach
                        </tr>
                    </template>
