...
<div {{ $attributes->merge(['class' => 'relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-200 hover:shadow-lg dark:border-slate-700/60 dark:bg-slate-900']) }}>
    ...
    <div class="px-5 pt-5 pb-1">
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
                ...
                @endif
                <p class="mt-2 text-3xl font-extrabold tracking-tight tabular-nums text-slate-900 dark:text-white">{{ $value }}</p>
            ...
            @if($icon)
                <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl"
                    ...
                    <span style="color: {{ $c[0] }}; font-size: 1.25rem;">{!! $icon !!}</span>
                </div>
            @endif
        </div>
    ...
    <div class="mt-1">
        <div id="{{ $chartId }}" data-chart-type="sparkline" data-config="{{ json_encode($config) }}"></div>
    </div>
...
@once
    @push('scripts')
        <script src="{{ asset('assets/js/card-chart/sparkline-stat-card.js') }}"></script>
    @endpush
@endonce
