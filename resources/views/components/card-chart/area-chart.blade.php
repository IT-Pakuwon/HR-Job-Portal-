...
@php
    ...
    $config = ['series' => is_string($series) ? json_decode($series,true) : $series, 'categories' => is_string($categories) ? json_decode($categories,true) : $categories, 'height' => (int)$height, 'color' => $color, 'stacked' => (bool)$stacked];
@endphp
...
<div {{ $attributes->merge(['class' => 'relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-200 hover:shadow-lg dark:border-slate-700/60 dark:bg-slate-900']) }}>
    ...
    <div class="px-2 pb-3 pt-1">
        <div id="{{ $chartId }}" data-chart-type="area" data-config="{{ json_encode($config) }}"></div>
    </div>
</div>

@once
    @push('scripts')
        <script src="{{ asset('assets/js/card-chart/area-chart.js') }}?v={{ filemtime(public_path('assets/js/card-chart/area-chart.js')) }}"></script>
    @endpush
@endonce
