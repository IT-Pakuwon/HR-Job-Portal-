...
@php
    ...
    $config = [
        ...
        'height'        => (int)$height,
        'color'         => $color,
        'peakThreshold' => (int)$peakThreshold,
    ];
@endphp
...
<div {{ $attributes->merge(['class' => 'relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-200 hover:shadow-lg dark:border-slate-700/60 dark:bg-slate-900']) }}>
    ...
    <div class="flex items-start justify-between px-5 pt-5 pb-1">
        ...
        <span class="mt-1 inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-bold"
              style="background:{{ $c[0] }}22;color:{{ $c[0] }}">
            <span class="inline-block h-1.5 w-1.5 rounded-full" style="background:{{ $c[0] }}"></span>
            Peak Hours
        </span>
    </div>

    <div class="px-2 pb-3 pt-1">
        <div id="{{ $chartId }}" data-chart-type="traffic" data-config="{{ json_encode($config) }}"></div>
    </div>
</div>

@once
    @push('scripts')
        <script src="{{ asset('assets/js/card-chart/traffic-chart.js') }}"></script>
    @endpush
@endonce
