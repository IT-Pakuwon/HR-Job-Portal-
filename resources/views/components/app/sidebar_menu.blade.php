    ...
    <aside x-show="sidebarOpen" x-transition:enter="transform transition ease-out duration-300"
        ...
        <div class="space-y-6 p-4">
            <ul class="space-y-1">
                ...
                <!-- ================= MODUL Report ================= -->
                @php
                    $reportMenu = $rootMenus->firstWhere('menu_id', 'REPORT');
                    $allowedIds = isset($allowedMenuIds) ? $allowedMenuIds->toArray() : [];

                    $visibleMenus = collect();

                    if ($reportMenu) {
                        $visibleMenus = $reportMenu->children->filter(function ($menu) use ($allowedIds) {
