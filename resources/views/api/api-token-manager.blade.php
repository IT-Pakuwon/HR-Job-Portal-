                    ...
                    <div class="mt-2 grid grid-cols-1 gap-4 md:grid-cols-2">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                ...
                                <span class="ms-2 text-xs text-gray-600 dark:text-gray-400">{{ $permission }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                        ...
                        @foreach ($this->user->tokens->sortBy('name') as $token)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="ms-2 flex items-center">
                                    ...
                                    @if (Laravel\Jetstream\Jetstream::hasPermissions())
                                        <button class="ms-6 cursor-pointer text-xs text-gray-400 underline"
                                            wire:click="manageApiTokenPermissions({{ $token->id }})">
                                            {{ __('Permissions') }}
                                        </button>
                                    @endif

                                    <button class="ms-6 cursor-pointer text-xs text-red-500"
