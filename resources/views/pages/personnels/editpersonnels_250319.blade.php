...
<div class="px-4 sm:px-6 lg:px-8 w-full max-w-9xl mx-auto">
    ...
    <div class="mt-2 mb-2 bg-white  dark:bg-gray-800 p-6 rounded-xl">
        ...
        <h2 class="text-2xl font-bold mb-4">Edit Personnel Requisition - {{ $personnel->docid }}</h2>
        ...
        <form id="personnelForm" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="cpnyid" class="w-full p-3 border rounded-lg select2">                        
                        @foreach($usercpny as $p)
                            <option value="{{ $p->cpnyid }}" {{ $p->cpnyid == $personnel->cpnyid ? 'selected' : '' }}>{{ $p->cpnyid }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-gray-700">Department</label>
                    <select name="departementid" class="w-full p-3 border rounded-lg select2">                       
                        @foreach($userdept as $p)
                            <option value="{{ $p->deptname }}" {{ $p->deptname == $personnel->departementid ? 'selected' : '' }}>{{ $p->deptname }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="job_level" class="w-full p-3 border rounded-lg select2">
                        @foreach($joblevel as $p)
                            <option value="{{ $p->title_level }}" {{ $personnel->job_level == $p->title_level ? 'selected' : '' }}>
                                {{ $p->title_level }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="immediate_superior" id="immediate_superior" value={{ $personnel->immediate_superior }} class="w-full p-2 border rounded">
                </div>
                <div>
                    <label class="block text-gray-700">State Position</label>
                    <input type="text" name="state_position" id="state_position" value={{ $personnel->state_position }} class="w-full p-2 border rounded">
                </div>
                <div>
                    <label class="block text-gray-700">Job Type</label>
                    <select name="job_type" id="job_type" class="w-full p-2 border rounded">                      
                ...
                <div>
                    ...
                    <textarea name="reason_vacancy" id="reason_vacancy" class="w-full p-2 border rounded">{{ $personnel->reason_vacancy }}</textarea>
                </div>
                <div>
                    <label class="block text-gray-700">Total Number Required</label>
                    <input type="number" name="required" id="required" value={{ $personnel->required }} class="w-full p-2 border rounded">
                </div>
                <div>
                    <label class="block text-gray-700">Actual</label>
                    <input type="number" name="actual" id="actual" value={{ $personnel->actual }} class="w-full p-2 border rounded">
                </div>
                <div>
                    <label class="block text-gray-700">The Actual Number</label>
                    <input type="number" name="total_actual" id="total_actual" value={{ $personnel->total_actual }} class="w-full p-2 border rounded">
                </div>
            </div>
            ...
            <div class="mt-6">
                    ...
                    <tbody id="responsibilitiesTable">
                        @foreach ($jobres as $key => $resp)
                            <tr>
                                ...
                                <td class="p-3 border">
                                    <input type="text" name="responsibilities[]" class="w-full p-2 border rounded" value="{{ $resp->job_responsibilities_descr }}">
                                </td>
                ...
                <button type="button" id="addResponsibilities" class="mt-3 px-4 py-2 bg-blue-500 text-white rounded">
                    + Add Responsibility
                </button>
            </div>
            ...
            <div class="mt-6">
                    ...
                    <tbody id="qualificationTable">
                        @foreach ($jobqua as $key => $resp)
                            <tr>
                                <td class="p-3 border text-center">{{ $key+1 }}</td>
                                <td class="p-3 border">
                                    <input type="text" name="qualification[]" class="w-full p-2 border rounded" value="{{ $resp->job_qualification_descr }}">
                                </td>
                                <td class="p-3 border text-center">
                ...
                <button type="button" id="addQualification" class="mt-3 px-4 py-2 bg-blue-500 text-white rounded">
                    + Add Qualification
                </button>
            </div>
                ...
                <div id="attachmentsContainer">
                    @foreach ($attachment as $attach)
                        <div class="attachment-row flex items-center gap-2" data-attachid="{{ $attach->id }}">
                            <a href="{{ url('/attachments/' . $attach->attachfile) }}" target="_blank" class="text-blue-600 underline">
                                📎 {{ $attach->name }}
                            </a>
                            <button type="button" class="removeAttachment2 bg-red-500 text-white px-3 py-1 rounded" data-id="{{ $attach->id }}">
                                X
                            </button>
                        </div>
                    @endforeach
                </div>
                <button type="button" id="addAttachment" class="mt-3 px-4 py-2 bg-blue-500 text-white rounded">
                    + Add Attachment
                </button>
            </div>
            ...
            <!-- Submit Button -->
            <div class="mt-6">
                <button type="submit" id="submitBtn" class="px-4 py-2 bg-blue-500 text-white rounded">
                    Submit Approval
                </button>
            </div>
        </form>
