<x-app-layout>
    <div class="max-w-9xl mx-auto w-full p-2">
        <div
            ...
            {{-- header + tombol add --}}
            <div
                class="flex flex-col gap-3 border-b border-gray-100 px-5 py-2 dark:border-white/[0.06] md:flex-row md:items-center md:justify-between">
                ...
                <form id="approvalForm">
                    ...
                    <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        <div>
                            ...
                            <select id="aprv_groupbiaya" name="aprv_groupbiaya"
                                class="w-full rounded-lg border px-3 py-2 dark:bg-gray-700" required>
                                <option value="">choose </option>
                                @foreach ($groupbiaya as $g)
                    ...
                    {{-- template options group biaya (hidden) --}}
                    <select id="groupBiayaOptionsTemplate" class="hidden">
                        <option value="">choose </option>
                        @foreach ($groupbiaya as $g)
                            <option value="{{ $g->groupbiaya_id }}">
                                {{ $g->groupbiaya_id }} - {{ $g->groupbiayadescr }}
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#aprv_doctype, #aprv_departementid, #aprv_cpnyid_select, #aprv_groupbiaya').select2({
                width: '100%',
                dropdownParent: $('#approvalModal')
            });

            // ==== Select2 untuk FILTER di atas tabel (di luar modal) ====
            $('#filterDoctype, #filterCompany, #filterDept, #filterGroupBiaya').select2({
                width: '100%'
            });
            ...
            applyColumnFilter('#filterGroupBiaya', 6);
            ...
            $('#clearUserFilters').on('click', function() {
                ...
                $('#filterGroupBiaya').val('').trigger('change');
            ...
            function loadDepartmentsByDoctype(doctype, selectedValue = null) {
                const url = "{{ route('approvalsgroupbiaya.departments') }}" + "?doctype=" + encodeURIComponent(doctype || '');
            ...
            $('#addApprovalBtn').click(function() {
                ...
                $('#aprv_groupbiaya').val('').trigger('change');
            ...
            $(document).on('click', '.editApprovalBtn', function() {
                ...
                $.get(`/approvals-groupbiaya/${id}/edit`, function(data) {
                    ...
                    $('#aprv_groupbiaya').val(data.aprv_groupbiaya).trigger('change');
            ...
            $('#approvalForm').submit(function(e) {
                ...
                const groupBiaya = $('#aprv_groupbiaya').val();
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        // reset form biar value lama tidak nyangkut
                        $('#approvalForm')[0].reset();
                        $('#id').val('');
                        $('#linesContainer').empty();
