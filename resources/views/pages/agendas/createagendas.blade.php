            ...
            <div class="max-w-9xl mx-auto w-full px-0 px-1 py-4 lg:px-8">
                <div class="gap-6">
                    <div
                        ...
                        <form id="agendaForm" class="flex flex-col gap-4" enctype="multipart/form-data">
                            @csrf
                            <div
                                ...
                                <div class="flex justify-between border-b dark:border-gray-600">
                                    <h2 class="mb-2 text-base font-bold">Create Agenda</h2>
                                </div>
                                <div
                                    class="mt-2 mt-2 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-2 dark:border-gray-600">
                                    <div class="flex items-center gap-4">
                                        <label
                                            class="required mb-1 block w-40 font-medium text-gray-700 dark:text-gray-300">Company</label>
                                        <select
                                    ...
                                    <div class="flex items-center gap-4">
                                        <label
                                            class="required mb-1 block w-40 font-medium text-gray-700 dark:text-gray-300">Department</label>
                                        <select
                                            class="w-full rounded-sm border border-gray-200/50 bg-gray-200/10 p-3 focus:ring focus:ring-blue-300 dark:bg-gray-800"
                                    ...
                                    <div class="flex items-center gap-4">
                                        <label
                                            class="required mb-1 block w-40 font-medium text-gray-700 dark:text-gray-300">Title</label>
                                    ...
                                    <div class="flex items-center gap-4">
                                        <label
                                            class="mb-1 block w-40 font-medium text-gray-700 dark:text-gray-300">Room</label>
                                        <select
                                            class="w-full rounded-sm border border-gray-200/50 bg-gray-200/10 p-3 focus:ring focus:ring-blue-300 dark:bg-gray-800"
                                            name="room_id">
                                            <option value="" disabled selected>Select</option>
                                            @foreach ($room as $p)
                                                <option value="{{ $p->room_id }}">{{ $p->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="flex items-center gap-4">
                                        <label
                                            class="mb-1 block w-40 font-medium text-gray-700 dark:text-gray-300">Zoom</label>
                                        <div
                                            class="w-full rounded-sm border border-gray-200/50 bg-gray-200/10 p-3 focus:ring focus:ring-blue-300 dark:bg-gray-800">
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="acc_id" value="1"
                                                    class="form-checkbox">
                                    ...
                                    <div class="flex items-center gap-4">
                                        <label
                                            class="required mb-1 block w-40 font-medium text-gray-700 dark:text-gray-300">Start
                                            Date</label>
                                    ...
                                    <div class="flex items-center gap-4">
                                        <label
                                            class="required mb-1 block w-40 font-medium text-gray-700 dark:text-gray-300">End
                                            Date</label>
                            ...
                            <div class="flex w-full flex-col gap-2 rounded-xl border-b bg-white dark:bg-gray-800">
                                ...
                                <div class="flex h-auto w-full flex-row justify-end gap-4 pl-4 pr-4">
                                    <div class="w-1/8 flex flex-col justify-start">
                                        <button type="button" id="cancelBtn" onclick="window.location.href='/agendas'"
            ...
            $('#agendaForm').submit(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#agendaForm')[0].reset(); // Reset form setelah submit
    ...
    <script>
        $(document).ready(function() {
            $('#agendaForm').submit(function(e) {
                ...
                let roomId = $('select[name="room_id"]').val();
                ...
                $.ajax({
                    url: "{{ route('agendas.checkRoomAvailability') }}", // Create a new route in your routes/web.php
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        room_id: roomId,
                    ...
                    success: function(response) {
                        if (response.status === 'booked') {
                            // Show popup if room is already booked
                            alert('Room is already booked for the selected time.');
                            $('#loadingSpinner').addClass('hidden');
                            $('#submitBtn').attr('disabled', false); // Enable button
                            $('#btnText').text(
                                'Submit Approval'); // Reset button text                  
                        } else {
                            // Proceed with form submission if room is available
                            submitAgendaForm(); // Proceed with original form submission logic
                        }
                    },
                    error: function(xhr) {
            ...
            function submitAgendaForm() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#btnText').text('Submit Approval');
                        $('#loadingSpinner').addClass('hidden'); // Sembunyikan spinner
                        toastr.success("Agenda Submit Successfully!");
                        window.location.href = "/agendas";
                    },
                    error: function(xhr) {
                        if (xhr.status === 422 && xhr.responseJSON.message) {
