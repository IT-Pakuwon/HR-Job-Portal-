                        ...
                        <div class="flex flex-col  bg-white dark:bg-gray-800 w-full rounded-xl shadow-sm">
                            <header class="px-6 py-4 flex justify-between items-center  rounded-t-2xl border-b bg-gray-50 border-gray-300/10 dark:border-gray-600">
                                <div class="flex justify-end max-w-5xl gap-2">
                                    <h1 class="text-base font-semibold text-gray-700 dark:text-gray-100">🆔 {{ $career->docid }}</h1>
                          
                                </div>
                                <div x-data="{ open: false }">
                                    <!-- Button to Open Modal -->
                                    <button @click="open = true" class="px-4 py-2 text-gray-500 dark:text-gray-200 rounded">
                                        <span class="text-l px-3 py-1 rounded-lg font-semibold
                                        @if($career->status === 'D') bg-gray-300/30 text-gray-600
                                            @elseif($career->status === 'P') bg-blue-300/30 text-blue-600
                                            @elseif($career->status === 'C') bg-green-300/30 text-green-600
                                            @elseif(in_array($career->status, ['X', 'R'])) bg-red-300/30 text-red-600
                                            @else bg-gray-500/30 text-gray-700 @endif">
                                        @php
                                            $statusText = match($career->status) {
                                                'D' => 'Revise',
                                                'P' => 'On Progress',
                                                'C' => 'Completed',
                                                'X' => 'Cancel',
                                    ...
                                    class="fixed top-16 right-0 w-full h-255 md:w-1/3 dark:bg-gray-700 bg-white   p-6 z-100">
                                        <header class="flex justify-end items-center px-6 py-1">
                                        ...
                                        {{-- Approval --}}
                                            ...
                                            {{-- Attachment --}}
                                            <div class="flex flex-col col-span-full sm:col-span-6 xl:col-span-12 border-b dark:border-gray-200/10 overflow-y-auto h-70">
                                                        ...
                                                        <div id="commentList" class="p-4 h-auto space-y-3 flex flex-col">
                                                            <template x-for="(comment, index) in comments" :key="index">
                                                                <div :class="comment.user === currentUser ? 'self-end bg-indigo-500 text-white' : 'self-start bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200'" class="p-3 rounded-lg max-w-xs  ">
                                                                    <p class="text-xs"><strong x-text="comment.user"></strong>: <span x-text="comment.text"></span></p>
                                                                </div>
                                                            </template>
                                    ...
                                    <!-- Background Overlay -->
                                    <div x-show="open" @click="open = false" class="fixed inset-0 z-0 bg-gray-500/10 bg-opacity-100" x-transition></div>
                                </div>
                            </header>
                            <!-- Main Content -->
                            <div class="p-4">
                                <div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        @php
                                            $jobDetails = [
                                                ['icon' => 'building-2', 'label' => 'Name', 'value' => $career->fullname],
                                                ['icon' => 'building', 'label' => 'Apply date', 'value' => $career->apply_date],
                                                ['icon' => 'briefcase', 'label' => 'Job Title', 'value' => $career->job_title],
                                                ['icon' => 'clipboard-list', 'label' => 'Job Type', 'value' => $career->job_type],
                                                ['icon' => 'user-check', 'label' => 'Immediate Superior', 'value' => $career->step],
                                                // ['icon' => 'user-check', 'label' => 'Status', 'value' => $career->status],
                                            ];
                                        @endphp
                                        ...
                                        <div class="grid grid-cols-2 gap-4 p-3 bg-gray-200/10 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $career->job_level }}</p>
                                                </div>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $career->state_position }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                            @php
                                                $jobNumbers = [
                                                    ['label' => 'Total Required', 'value' => $career->required],
                                                    ['label' => 'Actual', 'value' => $career->actual],
                                                    ['label' => 'Actual Number', 'value' => $career->total_actual],
                                                ];
                                            @endphp
                                    ...
                                    <div class="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                                        ...
                                        <p class="text-sm text-gray-900 dark:text-gray-100">{{ $career->reason_vacancy }}</p>
                                    </div>
                                </div>
                                        ...
                                        <ul x-show="isOpen" x-transition.opacity class="space-y-2 text-gray-700 dark:text-gray-300">
                                            <div class="flex flex-row gap-2 mb-2 mt-1">
                                                <li class="flex w-1/2 items-center space-x-2 bg-gray-100 dark:bg-gray-800 px-4 py-2 rounded-lg shadow-sm">
                                                    ...
                                                    <span class="font-medium">Education: {{ $career->education }}</span>
                                                </li>
                                                <li class="flex w-1/2 items-center space-x-2 bg-gray-100 dark:bg-gray-800 px-4 py-2 rounded-lg shadow-sm">
                                                    <span class="text-indigo-500 dark:text-indigo-400 text-sm">💼</span>
                                                    <span class="font-medium">Experience: Lebih dari {{ $career->experience_start }} tahun {{ $career->experience_end }} Year</span>
                                                </li>
                                            </div>
                ...
                $.ajax({
                    ...
                    error: function (xhr) {
                        ...
                        if (xhr.status === 403) {
                            ...
                            alert("Error: Unable to revise career status.");
                        }
                    },
                });
            });
