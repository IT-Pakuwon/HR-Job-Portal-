    ...
    <table class="w-full">
        <thead>
            <tr class="border-b border-gray-100 dark:border-gray-700/60">
                <th class="w-12 py-2.5 pl-5 pr-3 text-left text-sm font-bold uppercase tracking-widest text-gray-400">#</th>
                <th class="px-3 py-2.5 text-left text-sm font-bold uppercase tracking-widest text-gray-400">Checklist Item</th>
                <th class="py-2.5 pl-3 pr-5 text-right text-sm font-bold uppercase tracking-widest text-gray-400">Document</th>
            </tr>
        </thead>
            ...
            @foreach ($tr_checklist as $p)
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/20 {{ $uploaded ? 'bg-emerald-50/30 dark:bg-emerald-900/5' : '' }}">
                    <td class="py-3 pl-5 pr-3 {{ $uploaded ? 'border-l-2 border-emerald-400' : 'border-l-2 border-transparent' }}">
                        <span class="text-sm font-medium {{ $uploaded ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400' }}">{{ $p->step_order }}</span>
                    </td>
                    <td class="px-3 py-3 text-sm {{ $uploaded ? 'font-medium text-gray-800 dark:text-gray-100' : 'text-gray-600 dark:text-gray-300' }}">
                        {{ $p->checklist_descr }}
                        @if ($isAutoLinkItem)
                            <span class="ml-1.5 inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-sm font-semibold text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-300">Auto-linked</span>
                        @endif
                    </td>
                            ...
                            <div class="flex items-center justify-end gap-3">
                                @if ($prfPersonnel)
                                    <a href="{{ route('checklist.prf-pdf', $prfPersonnel->docid) }}" target="_blank"
                                        class="inline-flex items-center gap-1 text-sm font-medium text-indigo-600 transition hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300">
                                    ...
                                    <a href="{{ url('/showpersonnels/'.$prfHash) }}" target="_blank"
                                        class="inline-flex items-center gap-1 text-sm font-medium text-gray-500 transition hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                                ...
                                @else
                                    <span class="text-sm italic text-gray-400">No PRF document found</span>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center justify-end gap-3">
                                @if ($autoLinkUrl)
                                    <a href="{{ $autoLinkUrl }}" target="_blank"
                                        class="inline-flex items-center gap-1 text-sm font-medium text-emerald-600 transition hover:text-emerald-800 dark:text-emerald-400 dark:hover:text-emerald-300">
                                        <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
                                        Download
                                    </a>
                                @else
                                    <span class="text-sm italic text-gray-400">No {{ $autoLinkLabel }} document found</span>
                                @endif
                            </div>
                        @else
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @else
                                    <span class="text-sm italic text-gray-400">No document</span>
                                @endif
                                <button class="upload-btn inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-sm font-semibold text-gray-500 transition hover:border-indigo-300 hover:text-indigo-600 focus:outline-none active:scale-95 dark:border-gray-600 dark:text-gray-400 dark:hover:border-indigo-500 dark:hover:text-indigo-400"
    ...
    <div class="w-full max-w-sm rounded-lg bg-white shadow-xl dark:bg-gray-800">
        ...
        <form id="uploadForm" enctype="multipart/form-data" class="p-5">
            ...
            <input type="file" name="document" required
                class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 focus:border-gray-400 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white">
            <div class="mt-4 flex justify-end gap-2">
                ...
                <button type="submit"
                    class="rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-gray-700 focus:outline-none active:scale-95 dark:bg-white dark:text-gray-900">
                    Upload
                </button>
            </div>
        </form>
