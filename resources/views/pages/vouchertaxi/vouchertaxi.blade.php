                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            let url = `/showvouchertaxi/${row.eid}`;
                            let cls = 'inline-flex justify-center items-center w-[140px] px-3 py-1.5 text-sm leading-tight font-semibold text-white rounded text-center transition-colors duration-200 bg-gray-600 hover:bg-gray-700';

                            const text = data || '-';

                            if (row.status === 'D' && row.created_by === currentUser) {
                                url = `/editvouchertaxi/${row.eid}`;
                                cls = 'inline-flex justify-center items-center w-[140px] px-3 py-1.5 text-sm leading-tight font-semibold text-white rounded text-center transition-colors duration-200 bg-yellow-500 hover:bg-yellow-700';
                            }
