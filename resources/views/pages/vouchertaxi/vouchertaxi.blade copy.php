<x-app-layout>
                ...
                <table id="voucherTaxiTable" class="text-body w-full text-left text-sm rtl:text-right">
                    <thead class="text-body border-default-medium bg-neutral-secondary-soft rounded-base border-default border-b text-sm">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-sm font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">purpose</th>
    ...
    <div id="createVoucherModal"
      class="fixed inset-0 z-50 hidden bg-black/40 backdrop-blur-sm flex items-center justify-center overflow-y-auto">

        <div class="w-full max-w-4xl mt-16 mb-10 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl overflow-hidden">

            <form id="voucherTaxiForm" class="flex flex-col">
                ...
                <!-- BODY -->
                <div class="px-8 py-6 space-y-8 overflow-y-auto max-h-[70vh]">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <select name="cpny_id"
                                    class="w-full mt-1 px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-black focus:outline-none dark:border-gray-700 dark:bg-transparent"
                                    required>
                            ...
                            <div>
                                ...
                                <input type="text"
                                    ...
                                    class="w-full mt-1 px-3 py-2 rounded-lg border bg-gray-100 text-sm"
                                    readonly>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="text-xs text-gray-500">Type Trip *</label>
                            <div class="flex gap-3 mt-2">

                                <label class="px-4 py-2 rounded-full border border-gray-300 text-sm cursor-pointer
                                            hover:bg-gray-100 dark:border-gray-600">
                                    <input type="radio" name="type_trip" value="Return" checked class="hidden peer">
                                    <span class="peer-checked:text-white peer-checked:bg-black px-2 py-1 rounded-full">
                                        Return
                                    </span>
                                </label>
                                ...
                                <label class="px-4 py-2 rounded-full border border-gray-300 text-sm cursor-pointer
                                            hover:bg-gray-100 dark:border-gray-600">
                                    <input type="radio" name="type_trip" value="One Way" class="hidden peer">
                                    <span class="peer-checked:text-white peer-checked:bg-black px-2 py-1 rounded-full">
                                        One Way
                                    </span>
                                </label>
                            ...
                            <div>
                                <label class="text-xs text-gray-500">Topup *</label>
                                    <select name="user_topup" id="user_topup"
                                        class="w-full mt-1 px-3 py-2 rounded-lg border text-sm" required>
                ...
                <!-- FOOTER -->
                <div class="flex justify-between items-center px-8 py-5 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button type="submit"
                        class="bg-black text-white text-sm px-5 py-2 rounded-lg hover:opacity-90">
                        Submit
                    </button>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const deptSelect = document.querySelector('#department_id');
        const topupSelect = document.getElementById('user_topup');

        if (!deptSelect || !topupSelect) return;

        // init TomSelect
        const topupTom = new TomSelect('#user_topup', {
            maxOptions: 500,
            valueField: 'value',
            labelField: 'text',
            searchField: ['text'],
        ...
        deptSelect.addEventListener('change', function () {
            ...
            topupTom.clearOptions();

            // rebuild options based on department
            const allOptions = [...topupSelect.options];

            allOptions.forEach(opt => {

                if (!opt.value) return;

                const userDept = (opt.dataset.dept || '').trim();
            ...
            topupTom.clear(); // reset selection
            topupTom.refreshOptions(false);


    // ✅ ADD THIS
            setTimeout(() => {
                const dropdown = topupTom.dropdown_content;
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            document.querySelectorAll('#voucherTaxiForm [required]').forEach(el => {
                el.classList.add('req');

                // also mark label if exists
    ...
    <script>
        $(document).on('click', '.btn-edit-voucher', function() {
            ...
            const purpose = btn.attr('data-purpose') || '';
