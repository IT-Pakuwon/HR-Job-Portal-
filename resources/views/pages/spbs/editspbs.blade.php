                ...
                <form id="spbForm" class="flex flex-col gap-4" enctype="multipart/form-data"
                    ...
                    <div class="w-full rounded-xl bg-white p-6 dark:bg-gray-800">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                            ...
                            @php
                                $wtName  = optional($spb->worktype)->worktype_name ?? optional($spb->worktype)->name ?? $spb->worktypeid;
                                $swtName = optional($spb->subworktype)->subworktype_name ?? optional($spb->subworktype)->name ?? $spb->subworktypeid;
                                $jenisDisplay = trim(($wtName ? $wtName : '') . ($wtName && $swtName ? ' — ' : '') . ($swtName ? $swtName : ''));
                            @endphp
                            <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-col gap-2 lg:col-span-1">
                                ...
                                <select id="perpost" name="perpost"
                                    class="w-full rounded-lg border border-gray-300 bg-white p-2.5 text-gray-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300"
                                    required>
                                    <option value="{{ $year }}" {{ (string)$spb->budget_perpost === (string)$year ? 'selected' : '' }}>
                                        {{ $year }}
                                    </option>
                                    <option value="{{ $year + 1 }}" {{ (string)$spb->budget_perpost === (string)($year + 1) ? 'selected' : '' }}>
                                        {{ $year + 1 }}
                                    </option>
                                </select>
                            </div>
