...
<div class="grid auto-rows-fr grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 {{ $gridColsClass }}">
    ...
    {{-- Settlement All — admin only, system-wide view --}}
    ...
    <button type="button" class="text-left">
        <a href="#" class="status-filter group block h-full" data-status="ADMINALL">
            <div class="status-card flex h-full items-center gap-3 rounded-lg border border-purple-500 bg-purple-100/30 p-3 text-purple-600 transition-all duration-300 ease-in-out hover:-translate-y-1 hover:bg-purple-100 hover:shadow-md active:scale-95 dark:border-purple-500 dark:text-purple-400 dark:hover:bg-purple-500/20">
                <div class="flex h-6 w-6 shrink-0 items-center justify-center text-sm">🌐</div>
                <div class="flex min-w-0 flex-grow flex-col leading-tight"><p class="break-words text-sm font-medium">Settlement All</p></div>
    ...
    {{-- All Settlement — VPCOLLACCESS/VPLOYALTYACCESS/VPPRMTNACCESS only, company-wide view --}}
    @if($vplCompanyAccess)
    <button type="button" class="text-left">
        <a href="#" class="status-filter group block h-full" data-status="COMPANYALL">
            <div class="status-card flex h-full items-center gap-3 rounded-lg border border-teal-500 bg-teal-100/30 p-3 text-teal-600 transition-all duration-300 ease-in-out hover:-translate-y-1 hover:bg-teal-100 hover:shadow-md active:scale-95 dark:border-teal-500 dark:text-teal-400 dark:hover:bg-teal-500/20">
                <div class="flex h-6 w-6 shrink-0 items-center justify-center text-sm">🏢</div>
                <div class="flex min-w-0 flex-grow flex-col leading-tight"><p class="break-words text-sm font-medium">All Settlement</p></div>
...
<div id="createModal" class="fixed inset-0 z-[50] hidden items-center justify-center p-4">
    ...
    <div class="modal-panel modal-scroll relative z-10 flex max-h-[95vh] w-full max-w-6xl translate-y-4 scale-[0.98] flex-col overflow-y-auto rounded-lg border border-slate-200 bg-white opacity-0 shadow-2xl transition-all duration-200 dark:border-white/10 dark:bg-[#0f172a]">
        ...
        <div class="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/90 px-7 py-4 dark:border-white/10 dark:bg-[#0f172a]/90">
            <div>
                <h2 class="text-sm font-bold text-slate-900 dark:text-white">Create Settlement</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Settle a Completed Usage document.</p>
            </div>
        ...
        <div id="c_formView" class="bg-slate-50 p-6 dark:bg-[#0b1220]">
            <form id="createForm" class="space-y-6" enctype="multipart/form-data">
                ...
                <div class="overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-white/10 dark:bg-[#0f172a]">
                    ...
                    <div class="grid grid-cols-1 gap-5 p-6 md:grid-cols-4">
                        ...
                        <div class="md:col-span-4">
                            ...
                            <textarea name="settlement_remark" id="c_remark" rows="2" placeholder="Enter remarks..." required
                ...
                {{-- Warning: no lines loaded yet --}}
                <div id="c_lines_warning" class="hidden rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-400/30 dark:bg-amber-400/10 dark:text-amber-300">
                    <strong>⚠</strong> This Usage document has no remaining lines to settle.
                </div>

                {{-- Detail Lines --}}
                <div class="overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-white/10 dark:bg-[#0f172a]">
                    <div class="border-b border-slate-200 px-6 py-3 dark:border-white/10">
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-200">Settlement Details</h3>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm" id="c_detailTable">
                            ...
                            <tbody id="c_detailBody">
                                <tr id="c_detailEmptyRow"><td colspan="6" class="px-3 py-6 text-center text-sm text-slate-400">Select a Usage document to load its lines.</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
...
<div id="viewModal" class="fixed inset-0 z-[60] hidden items-center justify-center p-4">
    ...
    <div class="modal-panel modal-scroll relative z-10 flex max-h-[95vh] w-full max-w-7xl translate-y-4 scale-[0.98] flex-col overflow-y-auto rounded-lg border border-slate-200 bg-white opacity-0 shadow-2xl transition-all duration-200 dark:border-white/10 dark:bg-[#0f172a]">
        ...
        <div class="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/90 px-7 py-4 dark:border-white/10 dark:bg-[#0f172a]/90">
            <div>
                <h2 id="v_title" class="font-semibold text-slate-800 dark:text-white">Settlement Detail</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Settlement information &amp; approval workflow.</p>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-white/10 dark:bg-[#0f172a]">
                    ...
                    <div class="grid grid-cols-2 gap-4 p-4 md:grid-cols-3">
                        ...
                        <div><div class="text-xs text-slate-500 dark:text-slate-400">Event Date</div><div id="v_event_date" class="mt-1 text-sm font-medium text-slate-800 dark:text-slate-100"></div></div>
                ...
                <div class="overflow-hidden rounded-lg border border-blue-100 bg-blue-50 dark:border-blue-500/20 dark:bg-blue-500/10">
                    <div class="border-b border-blue-100 px-5 py-2 dark:border-blue-500/20">
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-blue-700 dark:text-blue-300">Settlement Details</h3>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            <thead class="border-b border-blue-100 dark:border-blue-500/20">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-right text-xs font-semibold text-blue-700 dark:text-blue-300">Qty Usage</th>
                                    <th class="px-4 py-2 text-right text-xs font-semibold text-blue-700 dark:text-blue-300">Qty Return</th>
                                    <th class="px-4 py-2 text-right text-xs font-semibold text-blue-700 dark:text-blue-300">Qty Settlement</th>
                                    <th class="px-4 py-2 text-right text-xs font-semibold text-blue-700 dark:text-blue-300">Sisa</th>
                                </tr>
                            </thead>
...
<div id="editModal" class="fixed inset-0 z-[70] hidden items-center justify-center p-4">
    ...
    <div class="modal-panel modal-scroll relative z-10 flex max-h-[95vh] w-full max-w-6xl translate-y-4 scale-[0.98] flex-col overflow-y-auto rounded-lg border border-slate-200 bg-white opacity-0 shadow-2xl transition-all duration-200 dark:border-white/10 dark:bg-[#0f172a]">
        ...
        <div class="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/90 px-7 py-4 dark:border-white/10 dark:bg-[#0f172a]/90">
            <div>
                <h2 id="e_title" class="text-sm font-bold text-slate-900 dark:text-white">Edit Settlement</h2>
        ...
        <div id="e_formView" class="bg-slate-50 p-6 dark:bg-[#0b1220]">
            <form id="editForm" class="space-y-6" enctype="multipart/form-data">
                ...
                <div class="overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-white/10 dark:bg-[#0f172a]">
                    <div class="border-b border-slate-200 px-6 py-3 dark:border-white/10">
                        <h3 class="text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-200">Settlement Information <span class="text-slate-400 font-normal normal-case text-[11px]">(read-only)</span></h3>
                    </div>
                    <div class="grid grid-cols-1 gap-5 p-6 md:grid-cols-3">
                        ...
                        <div>
                            <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200">Usage Doc</label>
                            <input type="text" id="e_usage_id_display" readonly class="h-11 w-full rounded-lg border border-slate-200 bg-slate-100 px-4 text-sm dark:border-white/10 dark:bg-white/[0.04]">
                        </div>
                        <div class="md:col-span-3">
                            <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200">Remark <span class="text-red-500">*</span></label>
                            <textarea name="settlement_remark" id="e_remark" rows="2" required
                                class="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm dark:border-white/10 dark:bg-white/[0.03] dark:text-white"></textarea>
                        </div>
                    </div>
                </div>
...
<script>
    window.VplSettlementConfig = {
        ...
        addAttach: (id) => `{{ url("settlementvp") }}/${id}/attachment/add`,
        data: (id) => `{{ url("settlementvp") }}/${id}/data`,
        update: (id) => `{{ url("settlementvp") }}/${id}/update`,
        cancel: (id) => `{{ url("settlementvp") }}/${id}/cancel`,
        approve: (id) => `{{ url("settlementvp") }}/${id}/approve`,
        reject: (id) => `{{ url("settlementvp") }}/${id}/reject`,
        revise: (id) => `{{ url("settlementvp") }}/${id}/revise`,
        message: (id) => `{{ url("settlementvp") }}/${id}/message`,
        show: (eid) => `{{ url("showsettlementvp") }}/${eid}`,
        initialId: {{ $initialId ?? 'null' }},
    };
</script>

<script src="{{ asset('assets/js/voucher-settlement/core.js') }}"></script>
<script src="{{ asset('assets/js/voucher-settlement/helper.js') }}"></script>
<script src="{{ asset('assets/js/voucher-settlement/datalist.js') }}"></script>
<script src="{{ asset('assets/js/voucher-settlement/modal.js') }}"></script>
<script src="{{ asset('assets/js/voucher-settlement/form.js') }}"></script>
<script src="{{ asset('assets/js/voucher-settlement/init.js') }}"></script>
