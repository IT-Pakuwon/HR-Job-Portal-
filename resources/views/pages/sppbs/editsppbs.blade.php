<x-app-layout>
                ...
                <form id="sppbForm" class="flex flex-col gap-4" enctype="multipart/form-data"
                    ...
                    <div class="flex w-full flex-col gap-2 rounded-xl bg-white p-4 shadow-sm dark:bg-gray-800">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-5">
                                ...
                                <select name="cpnyid" id="cpnyid"
                                    ...
                                    @foreach ($usercpny as $p)
                                        <option value="{{ $p->cpny_id }}"
                                            {{ $p->cpny_id == $sppb->cpny_id ? 'selected' : '' }}>
                                            {{ $p->cpny_id }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                @php
                                    $selectedBuId = old('business_unit_id', $sppb->business_unit_id ?? '');
                                    $selectedBuName = old('business_unit_name', $sppb->business_unit_name ?? '');
                                @endphp
                                ...
                                <select name="departementid" id="departementid"
                                    ...
                                    @foreach ($userdept as $p)
                                        <option value="{{ $p->department_id }}"
                                            {{ $p->department_id == $sppb->department_id ? 'selected' : '' }}>
                                            {{ $p->department_id }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="flex w-full">
                                    @php
                                        $selectedRT = old('requesttypeid', $sppb->requesttypeid);
                                        // sesuaikan relasi/nama field kamu:
                                        $selectedRTName = old(
                                            'requesttype_name',
                                            $sppb->requesttype_name ??
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-5">
                            ...
                            <div class="flex flex-row justify-between gap-2 xl:flex-col xl:justify-start">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">SPPB
                                    Emergency</label>
                                <div class="flex items-center gap-2">
                                    <input type="checkbox" id="is_urgent" name="is_urgent" value="1"
                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                        {{ old('is_urgent', $sppb->is_urgent ?? 0) ? 'checked' : '' }}>
                            ...
                            {{-- ITR ID --}}
                            <div class="flex flex-col gap-2">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">ITR ID</label>
                                <div class="flex w-full">
                                    <input type="text" name="itrecommendid" id="itrecommendid" readonly
                                        value="{{ old('itrecommendid', $sppb->itrecommendid ?? '') }}"
                                        class="w-full rounded-l-lg border border-gray-300 bg-white p-2.5 text-gray-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300"
                                        placeholder="Select ITR...">

                                    <input type="hidden" name="ticketid" id="ticketid"
                                        value="{{ old('ticketid', $sppb->ticketid ?? '') }}">
                    ...
                    <div id="itrModal"
                        ...
                        <div class="w-full max-w-6xl rounded-xl bg-white p-4 shadow-md dark:bg-gray-800">
                            <div class="mb-3 flex items-center justify-between border-b pb-2">
                                <h3 class="text-sm font-bold text-gray-800 dark:text-white">Select ITR Recommendation</h3>
    ...
    <script>
        ...
        $(function() {
            ...
            $(document).on('click', '.removeSppb', function() {
                ...
                if ($('#sppbTable .inventoryIdField').filter(function() {
    ...
    <script>
        ...
        $(function() {
            ...
            function validateItrForComputerItem() {
                let hasComputerItem = false;

                $('#sppbTable tr.sppb-row').each(function() {
                    const category = ($(this).find('.prodItemCategoryField').val() || '')
                        .toString()
                        .trim()
                        .toLowerCase();

                    if (category === 'komputer') {
                        hasComputerItem = true;
                        return false;
                    }
                });
                ...
                const itrid = ($('#itrecommendid').val() || '').trim();

                if (!itrid) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'ITR ID wajib diisi',
                        text: 'ITR ID wajib dipilih jika terdapat item dengan category Komputer.',
                        confirmButtonText: 'OK'
                    });

                    $('#itrecommendid').addClass('is-invalid').attr('aria-invalid', 'true');

                    if ($('#itrecommendid').next('.error-feedback').length === 0) {
                        $('#itrecommendid').after('<small class="error-feedback">ITR ID wajib diisi untuk item category Komputer.</small>');
                    }

                    $('html,body').animate({
                        scrollTop: $('#itrecommendid').offset().top - 120
                    }, 300);
            ...
            function submitSppbForm(isDraft) {
                ...
                if (!isDraft) {
                    ...
                    $('#sppbTable tr.sppb-row').each(function() {
                        ...
                        const $subVis = $tr.find(
                            '.locationDisplayField'); // sub juga tampil di field yang sama (combo)
    ...
    <script>
        $(function() {
            ...
            function loadInventory() {
                ...
                if (!cpnyid || !deptId) {
                    $tbody.html(
                        `<tr><td colspan="6" class="p-3 text-center">Select Company & Department first</td></tr>`
                    );
                    $invCount.text('');
            ...
            `).join('');
                    ...
                    .fail(function() {
                        $tbody.html(
                            `<tr><td colspan="6" class="p-3 text-center text-red-600">Failed to load inventory</td></tr>`
                        );
                        $invCount.text('');
                        $('#invPrev, #invNext').prop('disabled', true);
                    });
            ...
            $(document).on('click', '.chooseInventory', function() {
                ...
                // clear Budget (sesuai create kamu)
                currentRow.find(
                    '.coaIdField, .coaNameField, .activityIdField, .businessUnitIdField, .departmentFinIdField, .actDescrField'
                ).val('');

                // clear errors
                currentRow.find('.productNameField, .stock_unitField').removeClass('is-invalid').next(
    ...
    <script>
        ...
        $(document).on('click',
            '#inventoryModal button, #requestTypeModal button, #coaModal button, #uomModal button, #woModal button, #itrModal button, #modalLokasi button',
            function(e) {
                e.preventDefault();
                e.stopPropagation();
            });
    ...
    <script>
        $(function() {
            ...
            const initialBuId = @json(old('business_unit_id', $sppb->business_unit_id ?? ''));
            const initialBuName = @json(old('business_unit_name', $sppb->business_unit_name ?? ''));
