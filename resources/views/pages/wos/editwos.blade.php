...
<x-app-layout>
    <style>
        ...
        #loadingSpinnerContainer .loading-spinner {
            ...
            border: 4px solid transparent; border-top-color: #6366f1; animation: spin 1s linear infinite; position: relative;
        }
        #loadingSpinnerContainer .loading-spinner::after {
            content: ""; position: absolute; inset: 6px; border-radius: 50%;
            border: 4px solid transparent; border-left-color: #a5b4fc; animation: spinReverse .75s linear infinite;
        }
                ...
                <form id="woForm" class="flex flex-col gap-4" enctype="multipart/form-data"
                    ...
                    <div class="w-full rounded-xl bg-white p-6 shadow-md dark:bg-gray-800">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <select name="cpnyid"
                                    ...
                                    @foreach ($usercpny as $p)
                                        <option value="{{ $p->cpnyid }}" {{ $p->cpnyid == $prefill['cpnyid'] ? 'selected' : '' }}>
                                            {{ $p->cpnyid }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <select name="departementid"
                                    ...
                                    @foreach ($userdept as $p)
                                        <option value="{{ $p->deptname }}" {{ $p->deptname == $prefill['departementid'] ? 'selected' : '' }}>
                                            {{ $p->deptname }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            {{-- WO Type (AJAX by MsCategory: doctype=WO, categoryid=wotype; value=text) --}}
                            <div class="flex flex-col gap-2">
                            ...
                            {{-- WO Request (AJAX by MsCategory: doctype=WO, categoryid=worequest; value=text) --}}
                            <div class="flex flex-col gap-2">
                        ...
                        <div class="w-[95vw] max-w-2xl rounded-2xl bg-white p-6 dark:bg-gray-800">
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <select id="modal_worktypeid" class="mt-1 w-full rounded-lg border p-2.5 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300">
                                        <option value="">-- choose --</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="req block text-sm font-medium text-gray-700 dark:text-gray-300">Sub Worktype</label>
                                    <select id="modal_subworktypeid" class="mt-1 w-full rounded-lg border p-2.5 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300">
                                        <option value="">-- choose --</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="w-[95vw] max-w-2xl rounded-2xl bg-white p-6 dark:bg-gray-800">
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <select id="modal_location_id" class="mt-1 w-full rounded-lg border p-2.5 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300">
                                        <option value="">-- choose --</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="req block text-sm font-medium text-gray-700 dark:text-gray-300">Sub Location</label>
                                    <select id="modal_sub_location_id" class="mt-1 w-full rounded-lg border p-2.5 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300">
                                        <option value="">-- choose --</option>
                                    </select>
                                </div>
                            </div>
                                ...
                                @if (!empty($attachments) && count($attachments))
                                    @foreach ($attachments as $att)
                                        <a href="{{ $att->url }}" target="_blank"
                                           class="mb-2 inline-flex items-center gap-2 rounded border border-gray-300 p-2 text-indigo-600 hover:bg-gray-50 dark:border-gray-700 dark:text-indigo-400 dark:hover:bg-gray-700">
                                            📎 {{ $att->display_name }}
                                        </a>
                                    @endforeach
                                @endif
    ...
    <script>
        $(function () {
            ...
            function selectByText($sel, text){
                if (!text) return;
                const $opt = $sel.find('option').filter(function(){ return $(this).text().trim() === String(text).trim(); });
            ...
            function loadCategories($select, categoryid, selectedText=null) {
                ...
                $.getJSON(`/wos/ajax/categories/${categoryid}`)
                  .done(function(list){
                      ...
                      if (selectedText){ selectByText($select, selectedText); }
                  })
                  .fail(function(){ toastr.error('Gagal memuat kategori '+categoryid); });
            }
        ...
        $(function () {
            ...
            $('#btnJenisPekerjaan').on('click', function(){
                ...
                $.getJSON(`/wos/ajax/worktypes?${params}`)
                  ...
                  .fail(function(){ toastr.error('Gagal memuat Worktype.'); });
            });
            ...
            $('#modal_worktypeid').on('change', function(){
                ...
                $.getJSON(`/wos/ajax/subworktypes/${encodeURIComponent(wt)}?doctype=WO`)
                  .done(function(list){ list.forEach(it => $sub.append(new Option(it.text, it.value))); })
                  .fail(function(){ toastr.error('Gagal memuat Sub Worktype.'); });
            });
            ...
            $('#saveJenisPekerjaan').on('click', function(){
                ...
                if (!wtVal || !swVal) { toastr.error('Pilih Worktype dan Sub Worktype.'); return; }
        ...
        $(function () {
            ...
            $('#btnLokasi').on('click', function(){
                ...
                $.getJSON(`/wos/ajax/locations/${encodeURIComponent(cpny)}`)
                  ...
                  .fail(function(){ toastr.error('Gagal memuat Location.'); });
            });
            ...
            $('#modal_location_id').on('change', function(){
                ...
                $.getJSON(`/wos/ajax/sublocations/${encodeURIComponent(cpny)}/${encodeURIComponent(loc)}`)
                  .done(function(list){ list.forEach(it => $sub.append(new Option(it.text, it.value))); })
                  .fail(function(){ toastr.error('Gagal memuat Sub Location.'); });
            });
            ...
            $cpny.on('change', function(){ $('#location_id, #sub_location_id, #lokasi_display').val(''); });
        });
    </script>
    ...
    <script>
        $(function(){
            ...
            function addError($el, msg){
                ...
                if ($el.next('.error-feedback').length === 0) $el.after('<small class="error-feedback">'+msg+'</small>');
            }
            ...
            $('#woForm').on('submit', function(e){
                ...
                if($biaya.val() && isNaN(parseFloat($biaya.val()))) { addError($biaya, 'Biaya WO tidak valid.'); ok=false; }
