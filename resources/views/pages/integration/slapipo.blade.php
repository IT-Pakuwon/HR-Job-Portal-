...
<div class="rounded-xl border border-gray-200 bg-white p-4">
    ...
    <div class="mt-4 overflow-hidden rounded-xl border border-gray-200">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody id="slPoTbody" class="divide-y divide-gray-100">
                    <tr>
                        <td colspan="7" class="px-4 py-10 text-center text-gray-500">Belum ada data. Klik Load.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="px-4 py-3 text-xs text-gray-500 bg-white">
            Legend: P = ready (boleh diproses), D = waiting review, C = completed (tidak bisa diproses).
        </div>
    </div>
</div>
...
(() => {
    ...
    function setBusy(busy, title = 'Processing...', sub = 'Mohon tunggu...') {
        ...
        el.tbody.querySelectorAll('.slPoRowChk').forEach(chk => {
            if (busy) {
                chk.disabled = true;
            } else {
    ...
    function syncChkAllState() {
        const enabled = Array.from(el.tbody.querySelectorAll('.slPoRowChk')).filter(chk => !chk.disabled);
    ...
    function renderRows(rows) {
        ...
        if (!rows || rows.length === 0) {
            el.tbody.innerHTML = `
                <tr>
                    <td colspan="7" class="px-4 py-10 text-center text-gray-500">No data.</td>
                </tr>
            `;
            el.chkAll.checked = false;
            el.chkAll.indeterminate = false;
        ...
        el.tbody.innerHTML = rows.map(r => {
            ...
            const key = String(r.key ?? `${cpny}||${orderNo}`);
            ...
            return `
                <tr class="hover:bg-gray-50 ${rowClass}">
                    <td class="px-4 py-2">
                        <input type="checkbox"
                            class="slPoRowChk rounded border-gray-300"
                            value="${escapeHtml(key)}"
                            data-stage="${escapeHtml(stage)}"
                            ${disabled}>
                    </td>
                    <td class="px-4 py-2">${escapeHtml(cpny)}</td>
                    <td class="px-4 py-2">${escapeHtml(orderNo)}</td>
                    <td class="px-4 py-2">${escapeHtml(fmtDate(r.order_date ?? ''))}</td>
                    <td class="px-4 py-2">${escapeHtml(r.supplier_cd ?? '')}</td>
                    <td class="px-4 py-2">${escapeHtml(fmtDate(r.created_at ?? ''))}</td>
    ...
    async function loadData() {
        ...
        setBusy(true, 'Loading PO Solomon...', 'Sedang mengambil data PO Solomon.');
        ...
        try {
            ...
            setInfo('ok', `Loaded ${rows.length} PO. Ready(P): ${ready}, Waiting(D): ${waiting}, Completed(C): ${done}.`);
        } catch (err) {
            console.error(err);
            renderRows([]);
    ...
    async function processData() {
        ...
        const ids = Array.from(el.tbody.querySelectorAll('.slPoRowChk:checked'))
            .filter(chk => String(chk.dataset.stage ?? '').toUpperCase() === 'P')
            .map(chk => chk.value)
            .filter(v => v && v !== 'undefined' && !v.endsWith('||'));

        if (ids.length === 0) {
            setInfo('warn', 'Pilih minimal 1 PO status P untuk diproses. Status D/C tidak bisa.');
            return;
        }

        setBusy(true, 'Processing PO Solomon...', 'Sedang insert ke Solomon (P → C).');
        ...
        try {
            ...
            if (!resp.ok || !json.ok) {
                ...
                const detail = fail0
                    ? ` (${fail0.cpny_id || ''}||${fail0.order_no || ''}: ${fail0.error || ''})`
                    : '';
