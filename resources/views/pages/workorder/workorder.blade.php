...
<div class="mt-6  overflow-y-auto bg-white  dark:bg-gray-800 p-4 rounded-xl">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg">
        <table id="tasksTable" class="min-w-full rounded mt-5">
    ...
    <!-- Modal Create WI -->
    <div id="createWiModal" class="fixed z-10 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="p-4 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">                
                <select id="wo_priority" name="wo_priority" class="border p-2 rounded" required>
                    <option value="">Select Priority</option>                  
                    <option value="High">High</option>                   
                    <option value="Low">Low</option>                    
                </select>
                <select name="complaint_type" id="complaint_type" class="border p-2 rounded" required>
                    <option value="">Select Complaint Type</option>
                </select>
                <select name="work_type" id="work_type" class="border p-2 rounded" required>
                    <option value="">Select WO Type</option>
                </select>                
                <select name="sub_work_type" id="sub_work_type" class="border p-2 rounded" required>
                    <option value="">Select Sub Work Type</option>
                </select>               
                <select name="location_id" id="location_id" class="border p-2 rounded" required>
                    <option value="">Select Location</option>
                </select>
                <select name="sub_location_id" id="sub_location_id" class="border p-2 rounded" required>
                ...
                <textarea name="work_description" id="work_description" placeholder="Work Description" class="border p-2 rounded col-span-2"></textarea>      
                <select name="workers[]" id="workers" class="border p-2 rounded select2" multiple style="width: 100%; margin-bottom: 30px;" required>
                    <option value="">Select Workers</option>
                </select>        
            </div>
            <div class="mt-4 text-right">
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Save</button>
                <button type="button" class="bg-gray-300 px-4 py-2 rounded ml-2" id="closeWiModal">Cancel</button>
            </div>
            </form>
        </div>
        </div>
    ...
    $(function () {   
        ...
        var table = $('#tasksTable').DataTable({
            ...
            columns: [
                ...
                {data: 'taskpriority', name: 'taskpriority'},                            
    ...
    $('#wiForm').submit(function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if ($('#instructionTable').length && $.fn.DataTable.isDataTable('#instructionTable')) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.forEach(function (item) {
                    $select.append(`<option value="${item.complaintid}">${item.complaint_descr}</option>`);
                });
            },
            error: function (xhr) {
    ...
    function loadWOTypes(cpnyid, departementid) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.forEach(function (item) {
                    $select.append(`<option value="${item.worktype_id}">${item.worktype_descr}</option>`);
                });
            },
            error: function (xhr) {
    ...
    $('#work_type').on('change', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.forEach(function (item) {
                    $select.append(`<option value="${item.subworktype_id}">${item.subworktype_descr}</option>`);
                });
            },
            error: function (xhr) {
    ...
    function loadLocations(cpnyid) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.forEach(function (item) {
                    $select.append(`<option value="${item.location_id}">${item.location_descr}</option>`);
                });
            },
            error: function (xhr) {
    ...
    $('#location_id').on('change', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.forEach(function (item) {
                    $select.append(`<option value="${item.sublocation_id}">${item.sublocation_descr}</option>`);
                });
            },
            error: function (xhr) {
