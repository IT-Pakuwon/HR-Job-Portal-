        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:grid-rows-[minmax(0,auto)_1fr]">
            <div class="flex flex-col gap-8 lg:col-span-2 lg:row-span-1">
                <form id="returnForm" class="flex flex-col gap-4" enctype="multipart/form-data" method="POST"
                    action="{{ route('issue.return.store') }}">
                    @csrf
                    ...
                    <div class="w-full rounded-xl bg-white p-6 shadow-md dark:bg-gray-800">
                        <div class="mb-6 border-b border-gray-200 pb-4 dark:border-gray-700">
                            <h2 class="text-base font-extrabold text-gray-800 dark:text-white">Create Return (Issue)
                            </h2>
                        </div>
                                    ...
                                    <table class="min-w-full divide-y divide-gray-200 text-xs dark:divide-gray-700">
                                        ...
                                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                            @forelse($details as $d)
                                                <tr>
                                                    <td class="whitespace-nowrap px-4 py-2">{{ $d->inventoryid }}</td>
                                                    <td class="px-4 py-2">{{ $d->inventory_descr }}</td>
                                                    <td class="whitespace-nowrap px-4 py-2">{{ $d->siteid ?? '-' }}
                                                    </td>
                                                    <td class="whitespace-nowrap px-4 py-2">
                                                        {{ $d->location_id ?? '-' }}
                                                        @if (!empty($d->sub_location_id))
                                                            <span
                                                                class="text-gray-400">/</span>{{ $d->sub_location_id }}
                                                        @endif
                                                    </td>
                    ...
                    <div class="w-full rounded-xl bg-white p-6 shadow-md dark:bg-gray-800">
                        ...
                        <div class="flex w-full justify-end gap-4 pt-4">
                            <a href="{{ url()->previous() }}"
                                class="inline-flex items-center justify-center rounded-lg bg-red-600 px-6 py-3 text-sm font-semibold text-white hover:bg-red-700">
                                Cancel
                            </a>
            ...
            function validateMax() {
                ...
                $('.qtyReturn').each(function() {
                    ...
                    if (val > 0 && val - max > 1e-9) {
                        ok = false;
                        addErr($(this), 'Qty Return tidak boleh melebihi Qty Sisa Return (' + max.toFixed(
                            2) + ')');
                    }
                });
                return ok;
