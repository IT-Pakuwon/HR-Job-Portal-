<x-app-layout>
    <style>
        ...
        /* =========================
        TransferTable Styles (samakan dengan canvassTable)
        ========================= */

        /* Filter box */
        #transferTable_filter {
            margin-bottom: 20px;
            display: flex;
            justify-content: flex-start;
            align-items: center;
        ...
        /* Length dropdown */
        #transferTable_length {
            width: auto;
            display: flex;
            justify-content: flex-start;
        }

        #transferTable_length select {
            width: auto;
            padding: 5px;
            min-width: 80px;
            padding: 0.25rem 0.5rem;
        ...
        #transferTable_length select option {
            padding: 5px;
        }
        ...
        /* Column width mirror (opsional, samakan seperti canvassTable) */
        #transferTable th:nth-child(1),
        #transferTable td:nth-child(1) {
            width: 120px;
            text-align: center;
        }
        ...
        <div class="grid">
            ...
            <script>
                $(function() {
                    ...
                    const transferTable = $('#transferTable').DataTable({
                        ...
                        ajax: {
                            ...
                            type: "GET",
                            data: function(d) {
                                d.doc = docTypeFilter; // PAKAI FILTER YANG SAMA
                            }
                        },
                        order: [
                            [3, 'desc'], // doc_date desc
                            [0, 'desc'], // doc_no desc
                        ],
                        columns: [
                            // 0) DocID button
                            ...
                            // 1) assignpurchasing current (text saja)
                            { data: 'assignpurchasing', className: 'text-left' },

                            // 2) assignpurchasing NEW (select2)
                            {
                                data: null,
                                className: 'text-left',
                    ...
                    $('#btnTransferJobs').on('click', function() {
                        ...
                        $.post("{{ route('transferjobs.update') }}", { items })
                            .done(res => {
                                ...
                                // kalau Anda mau juga refresh assign list:
                                // table.ajax.reload(null, true);
                            })
                            .fail(xhr => {
                                const msg = xhr.responseJSON?.message || 'Gagal transfer. Cek input Anda.';
                                toastr.error(msg);
                            })
                ...
                $('.status-filter').on('click', function(e) {
                    e.preventDefault();
                    docTypeFilter = $(this).data('doc') || '';
                    table.ajax.reload(null, true);         // Assign List
                    transferTable.ajax.reload(null, true); // Transfer Jobs
                });
