        ...
        <div class="grid">            
            <div class="mt-6 rounded-2xl bg-white dark:bg-gray-800">
                ...
                <div class="overflow-x-auto p-6">
                    ...
                    <div id="tab-mine" class="tab-pane block">
                        ...
                        <tr>
                            <th class="px-6 py-3 text-left  text-xs font-semibold uppercase tracking-wider">Create CS</th>
            ...
            <script>
                ...
                function colSetWithoutCreate() {
                    ...
                    // 1) assigndate
                    { data:'assigndate', className:'text-center',
                        render:(v)=> v ? (isNaN(new Date(v)) ? v : new Date(v).toLocaleDateString('id-ID')) : '' },
                    // 2) doc_date
                    { data:'doc_date', className:'text-center',
                        render:(v)=> v ? (isNaN(new Date(v)) ? v : new Date(v).toLocaleDateString('id-ID')) : '' },
                    // 3) cpny_id
                    { data:'cpny_id', className:'text-center' },
                    // 4) created_by_name
                    { data:'created_by_name', className:'text-center', defaultContent:'-' },
                    // 5) assignpurchasing
                    { data:'assignpurchasing', className:'text-center', defaultContent:'' },
                    // 6) assignby
                    { data:'assignby', className:'text-center', defaultContent:'' },
                    // 7) department_id
                ...
                const tblMine = $('#tblMine').DataTable({
                    ...
                    ajax:{ url:"{{ route('csjobs.mine.json') }}", type:"GET", data:(d)=>{ d.doc = docTypeFilter; } },
                ...
                const tblAll = $('#tblAll').DataTable({
                    processing:true, serverSide:true, deferRender:true,
                    pageLength:25, lengthMenu:[10,25,50,100,250],
                    ajax:{ url:"{{ route('csjobs.all.json') }}", type:"GET", data:(d)=>{ d.doc = docTypeFilter; } },
                ...
                const tblRevision = $('#tblRevision').DataTable({
                    processing:true, serverSide:true, deferRender:true,
                    pageLength:25, lengthMenu:[10,25,50,100,250],
                    ajax:{ url:"{{ route('csjobs.revision.json') }}", type:"GET", data:(d)=>{ d.doc = docTypeFilter; } },
                    order:[[3,'desc'],[1,'desc']],
                    columns: colSetWithCreate(),
                    searchDelay:400, stateSave:true, responsive:true
                });
                ...
                const tblSppbjkt = $('#tblSppbjkt').DataTable({
                    processing:true, serverSide:true, deferRender:true,
                    pageLength:25, lengthMenu:[10,25,50,100,250],
                    ajax:{ url:"{{ route('csjobs.sppbjkt.progress.json') }}", type:"GET", data:(d)=>{ d.doc = docTypeFilter; } },
                    order:[[2,'desc'],[0,'desc']],
                    columns: colSetWithoutCreate(),
                    searchDelay:400, stateSave:true, responsive:true
                });
                ...
                function setActiveTab(key) {
                    $('.tab-btn').removeClass('active bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300');
                    $(`.tab-btn[data-tab="${key}"]`).addClass('active bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300');
