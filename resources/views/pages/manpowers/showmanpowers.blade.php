        ...
        <div class="grid">
                                ...
                                <header
                                    ...
                                    <div class="flex w-full justify-between gap-2">
                                        <h1 class="text-base font-semibold text-gray-700 dark:text-gray-100">🆔
                                            {{ $manpower->docid }}</h1>
                                        <span
                                            class="text-l @if ($manpower->status === 'D') bg-gray-300/30 text-gray-600
                                            @elseif($manpower->status === 'P') bg-blue-300/30 text-blue-600
                                            @elseif($manpower->status === 'C') bg-green-300/30 text-green-600
                                            @elseif(in_array($manpower->status, ['X', 'R'])) bg-red-300/30 text-red-600
                                            @else bg-gray-500/30 text-gray-700 @endif rounded-lg px-3 py-1 font-semibold">
                                            @php
                                                $statusText = match ($manpower->status) {
                                                    'D' => 'Revise',
                                                    'P' => 'On Progress',
                                                    'C' => 'Completed',
                                                    'X' => 'Cancel',
                                    ...
                                    <div>
                                            ...
                                            @php
                                                $jobDetails = [
                                                    [
                                                        'icon' => 'building-2',
                                                        'label' => 'Company',
                                                        'value' => $manpower->cpnyid,
                                                    ],
                                                    [
                                                        'icon' => 'building',
                                                        'label' => 'Department',
                                                        'value' => $manpower->departementid,
                                                    ],
                                                    [
                                                        'icon' => 'briefcase',
                                                        'label' => 'Job Title',
                                        ...
                                        <div
                                            ...
                                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                                                @php
                                                    $jobNumbers = [
                                                        ['label' => 'Required', 'value' => $manpower->required],
                                                        ['label' => 'Actual', 'value' => $manpower->actual],
                                                        ['label' => 'Total', 'value' => $manpower->total_actual],
                                                    ];
                                                @endphp
                                                ...
                                                @foreach ($jobNumbers as $job)
                                                    <div
                                                        ...
                                                        <span
                                                            class="text-sm text-gray-500 dark:text-gray-400">{{ $job['label'] }}</span>
                                                        <span
                                    ...
                                    <div class="overflow-hidden rounded-xl bg-white shadow-sm dark:bg-gray-800">
                                        <header
                                            ...
                                            <button @click="isOpen = !isOpen"
                                                ...
                                                <span x-text="isOpen ? 'Close' : 'See Details'"></span>
                                            </button>
                                        </header>
                                        <div class="p-4">
                                            <div x-show="isOpen" x-transition.opacity>
                                                <table
                                                    class="w-full border border-gray-300 text-left text-sm text-gray-700 dark:border-gray-600 dark:text-gray-300">
                                                    <thead
                                                        class="bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300">
                                                        <tr>
                                                            <th class="border px-4 py-2">#</th>
                                                            <th class="border px-4 py-2">Expected Date</th>
                                                    ...
                                                    <tbody>
                                                        @foreach ($manpowerdetail as $i => $p)
                                                            <tr class="border-t dark:border-gray-600">
                                                                <td class="border px-4 py-2 text-center">
                                                                    {{ $i + 1 }}</td>
                                                                <td class="border px-4 py-2">
                                                                    {{ $p->expected_employment_date }}</td>
                                                                <td class="border px-4 py-2">{{ $p->job_title }}</td>
                                                                <td class="border px-4 py-2">{{ $p->job_level }}</td>
                                                                <td class="border px-4 py-2 text-center">
                                                                    {{ $p->qty }}</td>
                                                                <td class="border px-4 py-2">{{ $p->reason_vacancy }}
                                                                </td>
                                                            </tr>
                                                        @endforeach
                                                    </tbody>
            ...
            <div id="rejectTaskModal" class="fixed inset-0 z-50 flex hidden items-center justify-center bg-black/50">
                <div class="w-full max-w-md rounded-lg bg-white p-4 dark:bg-gray-700">
                    <h2 class="mb-4 text-base font-semibold text-gray-800 dark:text-white">Reject Manpower</h2>
            ...
            <div id="reviseTaskModal" class="fixed inset-0 z-50 flex hidden items-center justify-center bg-black/50">
                <div class="w-full max-w-md rounded-lg bg-white p-4 dark:bg-gray-700">
                    <h2 class="mb-4 text-base font-semibold text-gray-800 dark:text-white">Revise Manpower</h2>
            ...
            <script>
                $(document).ready(function() {
                    let docid = "{{ $manpower->docid }}"; // Ambil task ID dari PHP ke JavaScript
                    loadComments(docid);
            ...
            <script>
                ...
                function approveManpower(docid) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                toastr.success("Manpower approved successfully!");
                        ...
                        error: function(xhr) {
                            ...
                            if (xhr.status === 403) {
                                toastr.error("You are not authorized to approve this manpower.");
                            } else {
                                toastr.error("Error: Unable to approve manpower.");
                            }
                        },
                        complete: function() {
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $(document).on("click", "#confirmRejectBtn", function() {
                        let docid = "{{ $manpower->docid }}"; // Ambil ID tugas dari modal detail
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    // Update status di modal manpower
                                    ...
                                    alert("Failed to reject manpower.");
                                }
                            },
                            error: function(xhr) {
                                console.error(xhr.responseText);
                                ...
                                if (xhr.status === 403) {
                                    ...
                                    alert("Error: Unable to reject manpower status.");
                                }
                            },
                        });
                    });
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    alert("Failed to revise manpower.");
                                }
                            },
                            error: function(xhr) {
                                console.error(xhr.responseText);
                                ...
                                if (xhr.status === 403) {
                                    ...
                                    alert("Error: Unable to revise manpower status.");
                                }
                            },
                        });
                    });
            ...
            <script>
                function checkApproval(docid, action) {
                    ...
                    $.ajax({
                        url: `/manpower/${docid}/check-approval/${action}`,
                        type: "GET",
                        success: function(response) {
                            if (response.canPerformAction) {
                                ...
                                if (action === "reject") {
                                    ...
                                    //     approveManpower(docid); // Jika approve, langsung jalankan proses approval
                                }
                            } else {
                                // Jika user tidak boleh melakukan aksi, tampilkan popup toastr
                                toastr.error("You are not authorized to " + action + " this manpower.");
                            }
                        },
                        error: function() {
