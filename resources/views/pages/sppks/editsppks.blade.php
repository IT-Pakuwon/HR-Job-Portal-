<x-app-layout>
        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:grid-rows-[minmax(0,auto)_1fr]">
            <div class="flex flex-col gap-8 lg:col-span-2 lg:row-span-1">
                ...
                <form id="sppkForm" class="flex flex-col gap-4" enctype="multipart/form-data"
                    action="{{ route('sppks.update', $hash) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="w-full rounded-xl bg-white p-4 dark:bg-gray-800">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-5">
                                ...
                                <select name="cpnyid" id="cpnyid"
                                    ...
                                    @foreach ($usercpny as $p)
                                        <option value="{{ $p->cpny_id }}"
                                            {{ $p->cpny_id == $sppk->cpny_id ? 'selected' : '' }}>
                                            {{ $p->cpny_id }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                @php
                                    $selectedBuId = old('business_unit_id', $sppk->business_unit_id ?? '');
                                    $selectedBuName = old('business_unit_name', $sppk->business_unit_name ?? '');
                                @endphp
                                ...
                                <select name="departementid"
                                    ...
                                    @foreach ($userdept as $p)
                                        <option value="{{ $p->department_id }}"
                                            {{ $p->department_id == $sppk->department_id ? 'selected' : '' }}>
                                            {{ $p->department_id }}
                                        </option>
                                    @endforeach
                                </select>
    ...
    <script>
        $(function() {
            ...
            const initialBuId = @json(old('business_unit_id', $sppk->business_unit_id ?? ''));
            const initialBuName = @json(old('business_unit_name', $sppk->business_unit_name ?? ''));
