<x-app-layout>
                ...
                <div class="rounded-xl bg-white duration-300 sm:w-1/2 md:w-full dark:bg-gray-800">
                    <header
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ url('/pdf_sppks') }}/{{ $hash }}" target="_blank">
                                <button
                            ...
                            @php
                                ...
                                $fields = [
                                    ...
                                    [
                                        'icon' => 'user-circle',
                                        'label' => 'Created User',
                                        'value' => ucwords(strtolower(optional($sppk->creator)->name)),
                                    ],
                                    [
                                        'icon' => 'truck', // or any vehicle icon you have
                                        'label' => 'Vehicle Name',
                                        'value' => ucwords(strtolower($sppk->namakendaraan)),
                                    ],
                                    [
                                        'icon' => 'identification',
                                        'label' => 'Vehicle Owner',
                                        'value' => ucwords(strtolower($sppk->pemilikkendaraan)),
                                    ],
                                    [
                                        'icon' => 'rectangle-stack',
                                        'label' => 'License Plate',
                                        'value' => $sppk->no_polisi,
                                    ],
                                    [
                                        'icon' => 'chart-bar',
                                        'label' => 'KM',
                                        'value' => number_format($sppk->km_kendaraan, 0, ',', '.'),
                                    ],
                                ];
                            @endphp
                        ...
                        <header
                            ...
                            <nav class="flex flex-grow">
                                <button @click="activeTab = 'attachment'"
                                    ...
                                    class="flex-1 px-4 py-1 text-center text-xs font-medium transition-colors duration-200">
                                    Attachment
                                </button>
    ...
    <script>
        function checkApproval(sppkid, action) {
            $.ajax({
                url: `/approval/${sppkid}/check/${action}?doctype=PK`,
                type: "GET",
                success: function(response) {
                    if (response.canPerformAction) {
    ...
    <script>
        $(function() {
            const listUrl = @json(route('attachments.list', ['doctype' => 'PK', 'refnbr' => $sppk->sppkid]));
            const uploadUrl = @json(route('attachments.upload', ['doctype' => 'PK', 'refnbr' => $sppk->sppkid]));

            function $tbody() {
                return $('#sppkAttachmentTbody');
            } // <tbody id="sppkAttachmentTbody">
            ...
            $('#btnUploadSppbAttachment').on('click', function() {
                const $form = $('#sppkAttachmentUploadForm')[0];
                const files = $('#sppkAttachFiles')[0].files;
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const sppkid = "{{ $sppk->sppkid }}"; // contoh: PK2501010001
            const doctype = "PK";

            loadApproval(sppkid, doctype);
        });
