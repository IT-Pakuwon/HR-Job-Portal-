<x-app-layout>
    ...
    <div class="mt-6 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-lg">
        ...
        <table id="assessmentsTable" class="w-full border-collapse table-fixed">
    ...
    <div id="appModal" class="fixed inset-0 flex hidden items-center justify-center bg-black/50 z-50">
        <div class="bg-white dark:bg-gray-700 p-6 rounded-lg w-1/3 relative">
            <h2 id="modalTitle" class="text-xl font-bold mb-4 text-gray-800 dark:text-white">Add Assessment</h2>
            <form id="appForm">
                <input type="hidden" id="id">
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-white">AssessmentID</label>
                    <input type="text" id="assessment_id" class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-white">AssessmentGroup</label>                   
                    <textarea class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700" rows="3" id="assessment_group" name="assessment_group"></textarea>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-white">Assessment Descr</label>                    
                    <textarea class="w-full border px-3 py-2 rounded-lg dark:bg-gray-700" rows="3" id="assessment_descr" name="assessment_descr"></textarea>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-white">AssessmentScore</label>
                    <input type="text" id="assessment_score" class="number-only w-full border px-3 py-2 rounded-lg dark:bg-gray-700">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-white">Step Order Group</label>
                    <input type="text" id="step_order_group" class="number-only w-full border px-3 py-2 rounded-lg dark:bg-gray-700">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-white">Step Order</label>
                    <input type="text" id="step_order" class="number-only w-full border px-3 py-2 rounded-lg dark:bg-gray-700">
                </div>
    ...
    $(document).ready(function () {
        let table = $('#assessmentsTable').DataTable({
            ...
            serverSide: false,
            columnDefs: [
                { width: "120px", targets: 0 },  // Lebar kolom Actions
                { width: "120px", targets: 3 }   // Lebar kolom Status
            ],
            columns: [
                {
                    data: 'id',
                ...
                { data: 'assessment_id', width: '80px' },
                { data: 'assessment_group', width: '300px' },
                { data: 'assessment_descr', width: '350px' },
                { data: 'assessment_score', width: '50px' },
                { data: 'step_order_group', width: '50px' },
                { data: 'step_order', width: '50px' },
                {
...
<script>
    ...
    <style>
        ...
        #assessmentsTable th:nth-child(1), #assessmentsTable td:nth-child(1) {
            width: 120px;
            text-align: center;
        }
        #assessmentsTable th:nth-child(4), #assessmentsTable td:nth-child(4) {
            width: 120px;
            text-align: center;
        }
    </style>
    <style>
        ...
        #assessmentsTable th, #assessmentsTable td {
