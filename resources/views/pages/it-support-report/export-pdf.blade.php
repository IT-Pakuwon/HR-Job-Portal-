...
@php
    $moduleLabels = ['TICKET' => 'Ticket Support', 'RECOMMENDATION' => 'IT Recommendation', 'ACCESS' => 'Access Request'];
    $moduleLabel = $moduleLabels[$module] ?? 'Ticket Support';
    $s = $summary;
@endphp

<div class="rpt-header">
    <h1>IT Support Report — {{ $moduleLabel }}</h1>
    <div class="meta">
...
<div class="section">
    <div class="sec-head">{{ $moduleLabel }} List ({{ count($tableRows) }})</div>
    <table class="dt">
        <thead>
            <tr>
                <th>Doc ID</th>
        ...
        <tbody>
            @forelse($tableRows as $i => $r)
                <tr class="{{ $i % 2 === 1 ? 'ev' : '' }}">
                    ...
                    <td>{{ $r['issue'] }}</td>
                    <td>
                        <span class="status-pill {{ in_array($r['status'], ['COMPLETED', 'FINISHED']) ? '' : 'op' }}">{{ $r['status'] }}</span>
                    </td>
                </tr>
            @empty
...
<div class="rpt-footer">
    <div class="ft-l">IT Support Report</div>
