<x-app-layout>
    ...
    <div class="max-w-9xl mx-auto p-2">
        <x-breadcrumb :items="[
            ...
            ['label' => 'IM Budget', 'url' => route('imbudgets')],
            ['label' => 'Show Details'],
        ]">
            <x-approval-actions
                :status="$imbudget->status"
        ...
        <div class="flex w-full flex-col gap-4 overflow-hidden sm:col-span-1 lg:row-span-1 xl:row-span-1 xl:flex-col">
            <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
                ...
                <div class="flex h-[400px] flex-col overflow-y-auto rounded-xl bg-white dark:bg-gray-800">
                    <header
                        ...
                        @php
                            $statusText = match ($imbudget->status) {
                                'D' => 'Revise',
                                'P' => 'On Progress',
                                'C' => 'Completed',
                                'X' => 'Cancelled',
                            ...
                            $statusClasses = match ($imbudget->status) {
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ url('/pdf_imbudgets') }}/{{ $hash }}" target="_blank">
                                <button
            ...
            <div class="flex w-full flex-col rounded-xl bg-white dark:bg-gray-800">
                <header
                    ...
                    <h2 class="text-base font-semibold">📝 IMBudget Detail</h2>
                </header>
    ...
    <script>
        $(document).ready(function() {
            ...
            attachMentionAutocomplete({
                ...
                fetchUrlFn: () => `/mentionable-users/${doctype}/${imbudgetid}`,
            });
            ...
            function addComment() {
                ...
                $.ajax({
                    url: `/comments/${doctype}/${imbudgetid}`,
                    type: 'POST',
                    data: {
                        comment: input,
                        _token: '{{ csrf_token() }}'
    ...
    <script>
        ...
        function approveIMBudget(imbudgetid) {
            ...
            $.ajax({
                url: `/imbudget/${imbudgetid}/approve`,
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 403) {
                        toastr.error("You are not authorized to approve this imbudget.");
                    } else {
                        toastr.error("Error: Unable to approve imbudget.");
                    }
                },
                complete: function() {
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on("click", "#rejectBtn", function() {
                ...
                let imbudgetid = "{{ $imbudget->imbudgetid }}";
                checkApproval(imbudgetid, "reject");
            ...
            $(document).on("click", "#confirmRejectBtn", function() {
                let imbudgetid = "{{ $imbudget->imbudgetid }}"; // Ambil ID tugas dari modal detail
                ...
                $.ajax({
                    url: `/imbudget/${imbudgetid}/reject`,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        docid: imbudgetid,
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 403) {
                            ...
                            alert("Error: Unable to reject imbudget status.");
                        }
                    },
                });
            });
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on("click", "#reviseBtn", function() {
                ...
                let imbudgetid = "{{ $imbudget->imbudgetid }}";
                checkApproval(imbudgetid, "revise");
            ...
            $(document).on("click", "#confirmReviseBtn", function() {
                ...
                $.ajax({
                    url: `/imbudget/${imbudgetid}/revise`,
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        docid: imbudgetid,
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 403) {
                            ...
                            alert("Error: Unable to revise imbudget status.");
                        }
                    },
                });
            });
    ...
    <script>
        function checkApproval(imbudgetid, action) {
            $.ajax({
                url: `/approval/${imbudgetid}/check/${action}?doctype=IM`,
                type: "GET",
                success: function(response) {
                    if (response.canPerformAction) {
                        ...
                        toastr.error("You are not authorized to " + action + " this IMBudget.");
                    }
                },
                error: function() {
