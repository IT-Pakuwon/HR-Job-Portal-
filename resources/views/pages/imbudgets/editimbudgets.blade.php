<x-app-layout>
                                       ...
                                       <table class="mb-4 mt-3 w-full">
                                            ...
                                            <tbody id="imbudgetTable">
                                                ...
                                                <tr class="imbudget-row" data-detail-id="{{ $d->id }}">
                                                    ...
                                                    <input type="text" class="w-full border-none bg-transparent text-center"
                                                        value="{{ $d->budget_account_id }}" readonly>
                                                    <input type="hidden" name="budget_account_id[]" value="{{ $d->budget_account_id }}">
                                                    </td>
                                                    <td class="border p-3 text-center">
                                                    <input type="text" class="w-full border-none bg-transparent text-center"
                                                        value="{{ $d->budget_activity_id }}" readonly>
                                                    <input type="hidden" name="budget_activity_id[]" value="{{ $d->budget_activity_id }}">
                                                    </td>
                                                    <td class="border p-3">
                                                    <input type="text" class="w-full border-none bg-transparent"
                                                        value="{{ $d->budget_activity_descr }}" readonly>
                                                    <input type="hidden" name="budget_activity_descr[]" value="{{ $d->budget_activity_descr }}">
                                                    </td>

                                                    {{-- angka hasil agregasi/perhitungan (readonly) --}}
                                                    <td class="border p-3 text-right">
                                                    <input type="text" class="w-full border-none bg-transparent text-right amountExpenseField"
                                                        value="{{ $fmt($d->amount_expense ?? 0) }}" readonly>
                                                    <input type="hidden" name="amount_expense[]" value="{{ (float)($d->amount_expense ?? 0) }}">
                                                    </td>
                                                    <td class="border p-3 text-right">
                                                    <input type="text" class="w-full border-none bg-transparent text-right budgetRemainField"
                                                        value="{{ $fmt($d->budget_remain ?? 0) }}" readonly>
                                                    <input type="hidden" name="budget_remain[]" value="{{ (float)($d->budget_remain ?? 0) }}">
                                                    </td>
                                                    <td class="border p-3 text-right">
                                                    <input type="text" class="w-full border-none bg-transparent text-right budgetNeededField"
                                                        value="{{ $fmt($d->budget_needed ?? 0) }}" readonly>
                                                    <input type="hidden" name="budget_needed[]" value="{{ (float)($d->budget_needed ?? 0) }}">
                                                    </td>
                                                    ...
                                                    @endphp
                                                    <input type="text" name="budget_requested[]" class="reqField w-full border rounded-md px-2 py-1 text-right"
                                                        value="{{ $reqDisp }}" inputmode="decimal" autocomplete="off" placeholder="0,00">
                                                    </td>
                                                    <td class="border p-3">
                                                    <input type="text" name="note[]" class="w-full border rounded-md px-2 py-1"
                                                        value="{{ $d->note }}">
                                                    </td>                                                  
                                                ...
                                                <tr class="imbudget-row">
                                                    <td colspan="14" class="border p-4 text-center text-sm text-gray-500">
    ...
    <script>
        ...
        $('#imbudgetTable tr.imbudget-row').each(function(){
            ...
            $reqVis.val($reqVis.val().replace(/\./g,'').replace(/,/g,',')); // biar tampilan tetap id (opsional)

            if (hasCoa) anyActive = true;

            if (hasCoa && reqNum < 0) {
            $reqVis.addClass('is-invalid').after('<small class="error-feedback">Budget Requested tidak boleh negatif.</small>');
            anyInvalid = true;
            }
        });
    ...
    <script>
        ...
        $(document).on('paste', 'input[name="budget_requested[]"]', function () {
            const el = this;
            setTimeout(() => { el.value = sanitizeNumericInput(el.value); }, 0);
        });
