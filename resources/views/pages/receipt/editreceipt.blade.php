 <x-app-layout>
         ...
         <div class="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:grid-rows-[minmax(0,auto)_1fr]">
             <div class="flex flex-col gap-8 lg:col-span-2 lg:row-span-1">
                 ...
                 <form id="receiptEditForm" class="flex flex-col gap-4" enctype="multipart/form-data">
                     ...
                     <div class="w-full rounded-xl bg-white p-4 shadow-md dark:bg-gray-800">
                         <div class="mb-6 border-b border-gray-200 pb-4 dark:border-gray-700">
                             <h2 class="text-base font-extrabold text-gray-800 dark:text-white">
                                 Edit Receipt
                                 <span
                                     class="ml-2 rounded bg-gray-100 px-2 py-0.5 text-xs font-medium dark:bg-gray-700">
                                     ({{ strtoupper($rcp->receipttype) }})
                                 </span>
                             </h2>
                             <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Hanya Qty & Attachment yang dapat
                                 diubah saat status Revise.</p>
                         </div>

                         <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                             <div class="flex flex-col gap-2">
                                 <label class="block text-xs font-medium text-gray-600 dark:text-gray-300">Receipt
                                     Nbr</label>
                     ...
                     <div class="flex w-full flex-col gap-2 rounded-xl border-b">
                                     ...
                                     <table class="min-w-full divide-y divide-gray-200 text-xs dark:divide-gray-700">
                                         <thead class="bg-gray-50 dark:bg-gray-700">
                                             <tr>
                                                 ...
                                                 <th
                                                     ...
                                                     {{ $isReturn ? 'Qty Return (Current)' : 'Qty Received (Current)' }}
                                                 </th>
                                                 <th
                                                 ...
                                                 <th
                                                     class="px-4 py-2 text-right font-semibold text-gray-600 dark:text-gray-300">
                                                     {{ $isReturn ? 'Qty Return Edit' : 'Qty Received Edit' }}</th>
                                                 <th
                                                     class="px-4 py-2 text-left font-semibold text-gray-600 dark:text-gray-300">
                                                     Site</th>
                                             </tr>
                                         ...
                                         <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                             @forelse($details as $d)
                                                 @php
                                                     $currentQty = $isReturn
                                                         ? (float) ($d->qty_return ?? 0)
                                                         : (float) ($d->qty_received ?? 0);
                                                     $currentQtyStr = rtrim(
                                                         rtrim(number_format($currentQty, 2, '.', ''), '0'),
                                                         '.',
                                                     );
                                                 @endphp
                                                 <tr>
                                                     <td class="px-4 py-2">{{ $d->receipt_no ?? $loop->iteration }}
                                                     </td>
                                                     <td class="px-4 py-2">{{ $d->inventoryid }}</td>
                                                     <td class="px-4 py-2">{{ $d->inventory_descr }}</td>
                                                     <td class="px-4 py-2 text-right">
                                                         {{ number_format($currentQty, 2) }}</td>
     ...
     <script>
         $(function() {
             ...
             $(document).on('input', '.qtyEdit', function() {
