        ...
        <div class="grid">
                            ...
                            <header
                                ...
                                <div x-data="{ open: false }">
                                    ...
                                    <button @click="open = true"
                                        class="rounded px-4 py-2 text-gray-500 dark:text-gray-200">
                                        <span
                                            class="text-l @if ($news->status === 'D') bg-gray-300/30 text-gray-600
                                            @elseif($news->status === 'P') bg-blue-300/30 text-blue-600
                                            @elseif($news->status === 'C') bg-green-300/30 text-green-600
                                            @elseif(in_array($news->status, ['X', 'R'])) bg-red-300/30 text-red-600
                                            @else bg-gray-500/30 text-gray-700 @endif rounded-lg px-3 py-1 font-semibold">
                                            @php
                                                $statusText = match ($news->status) {
                                                    'D' => 'Revise',
                                                    'P' => 'On Progress',
                                                    'C' => 'Completed',
                                                    'X' => 'Cancel',
                                    ...
                                    <div x-show="open"
                                        ...
                                        {{-- Approval --}}
                                        <div
                                            class="h-70 mt-2 flex w-full flex-col justify-center overflow-y-auto border-b dark:border-gray-200/10">
                                            <header
                                        ...
                                        {{-- Attachment --}}
                                        <div
                                            class="h-70 col-span-full flex flex-col overflow-y-auto border-b sm:col-span-6 xl:col-span-12 dark:border-gray-200/10">
                                        ...
                                        <div
                                            class="h-70 col-span-full flex flex-col overflow-y-auto border-b sm:col-span-6 xl:col-span-12 dark:border-gray-200/10">
                                            <div x-data="{ isOpen: true, comments: [], newComment: '', currentUser: 'User1' }"
                                                class="mt-4 flex w-full flex-col justify-center">
                                                <header
                                                    class="flex items-center justify-between overflow-y-auto px-5 pb-2 pt-4"
                                                    @click="isOpen = !isOpen">
                                                    <h2
                                                ...
                                                <div x-show="isOpen"
                                                    ...
                                                    <div id="commentList" class="flex h-auto flex-col space-y-3 p-4">
                                                        <template x-for="(comment, index) in comments"
                                                            ...
                                                            <div :class="comment.user === currentUser ?
                                                                ...
                                                                <p class="text-sm"><strong
                                                                        x-text="comment.user"></strong>: <span
                                    ...
                                    <div x-show="open" @click="open = false"
                                        class="fixed inset-0 z-0 bg-gray-500/10 bg-opacity-100" x-transition></div>
                                </div>
                            </header>
                            <!-- Main Content -->
                            <div class="p-4">
                                <div>
                                    <div
                                        ...
                                        <h3 class="mb-2 text-lg font-semibold text-gray-700 dark:text-gray-200">Title
                                        </h3>
                                        <p class="text-base text-gray-900 dark:text-gray-100">{{ $news->title }}</p>
                                    </div>
                                    <!-- Reason for Vacancy -->
                                    <div
                                        class="mt-6 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
                                        <h3 class="mb-2 text-lg font-semibold text-gray-700 dark:text-gray-200">
                                            Description</h3>
                                        <p class="text-base text-gray-900 dark:text-gray-100">{!! $news->description !!}
                                        </p>
                                    </div>
                                </div>
            ...
            <script>
                $(document).ready(function() {
                    let docid = "{{ $news->docid }}"; // Ambil task ID dari PHP ke JavaScript
                    loadComments(docid);
                ...
                function approveNews(docid) {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            ...
                            if (xhr.status === 403) {
                                toastr.error("You are not authorized to approve this news.");
                            } else {
                                toastr.error("Error: Unable to approve news.");
                            }
                        },
                        complete: function() {
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $(document).on("click", "#confirmRejectBtn", function() {
                        let docid = "{{ $news->docid }}"; // Ambil ID tugas dari modal detail
                        ...
                        $.ajax({
                            ...
                            error: function(xhr) {
                                ...
                                if (xhr.status === 403) {
                                    ...
                                    alert("Error: Unable to reject news status.");
                                }
                            },
                        });
                    });
                        ...
                        $.ajax({
                            ...
                            error: function(xhr) {
                                ...
                                if (xhr.status === 403) {
                                    ...
                                    alert("Error: Unable to revise news status.");
                                }
                            },
                        });
                    });
            ...
            <script>
                function checkApproval(docid, action) {
                    ...
                    $.ajax({
                        url: `/news/${docid}/check-approval/${action}`,
                        type: "GET",
                        success: function(response) {
                            if (response.canPerformAction) {
                                ...
                                if (action === "reject") {
                                    ...
                                    //     approveNews(docid); // Jika approve, langsung jalankan proses approval
                                }
                            } else {
                                // Jika user tidak boleh melakukan aksi, tampilkan popup toastr
                                toastr.error("You are not authorized to " + action + " this news.");
                            }
                        },
                        error: function() {
