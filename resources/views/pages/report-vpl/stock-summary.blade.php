<div class="space-y-4">
    ...
    {{-- FILTER PANEL --}}
    <div class="rounded-2xl border border-gray-200 bg-linear-to-br from-gray-50 to-indigo-50/30 p-6 shadow-sm dark:border-gray-700 dark:from-gray-800/40 dark:to-indigo-900/10">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 items-end">
            ...
            <div class="space-y-1">
                ...
                <select id="stocksum_cpnyid" class="form-input w-full">
            ...
            <div class="space-y-1">
                ...
                <select id="stocksum_month" class="form-input w-full">
            ...
            <div class="space-y-1">
                ...
                <select id="stocksum_year" class="form-input w-full">
            ...
            <div class="flex gap-2">
                <button type="button" id="stocksum_load" class="flex w-full items-center justify-center gap-2 rounded-lg bg-gray-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-black hover:shadow active:scale-[0.98] dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-white">
                    <i class="fa-solid fa-magnifying-glass-chart"></i> Show Report
                </button>
                <button type="button" id="stocksum_export" class="flex items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-800 shadow-sm transition hover:border-gray-400 hover:bg-gray-50 active:scale-[0.98] dark:border-gray-600 dark:bg-transparent dark:text-gray-200 dark:hover:bg-gray-700/40">
...
<script>
    (function () {
        const jsonUrl = @json(route('report.vpl.json', ['type' => 'stock-summary']));
        const exportUrl = @json(route('report.vpl.export', ['type' => 'stock-summary']));

        function currentParams() {
            return new URLSearchParams({
                cpnyid: $('#stocksum_cpnyid').val(),
                month: $('#stocksum_month').val(),
                year: $('#stocksum_year').val(),
            });
        }
        ...
        $(document).on('click', '#stocksum_load', loadStockSummary);

        $(document).on('click', '#stocksum_export', function () {
