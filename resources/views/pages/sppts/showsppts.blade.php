<x-app-layout>
            ...
            <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
                    ...
                    <div class="flex flex-1 flex-col overflow-y-auto px-4 py-[8px]">
                        <div class="grid grid-cols-2 gap-x-8 gap-y-1 text-sm sm:grid-cols-2">
                            ...
                            <div class="{{ $row }}">
                                ...
                                <span class="{{ $value }}">{{ $sppt->cpny_id }}</span>
                            </div>
                            ...
                            <div class="{{ $row }}">
                                ...
                                <span class="{{ $value }}">{{ $sppt->department_id }}</span>
                            </div>
                ...
                <div x-data="{ activeTab: 'attachment', tabsOpen: true }" class="flex flex-col overflow-y-auto rounded-xl bg-white dark:bg-gray-800"
                    ...
                    <div class="flex max-h-[100%] flex-1 flex-col overflow-y-auto">
                        ...
                        {{-- Tabs Content --}}
                        <div class="flex flex-1 flex-col overflow-y-auto rounded-xl bg-white dark:bg-gray-800" :class="{ 'hidden lg:flex': !tabsOpen }">
                            {{-- Approval tab --}}
            ...
            <div class="flex w-full flex-col rounded-xl bg-white dark:bg-gray-800">
                <header
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('sppt.export', $sppt->id) }}"
                        ...
                        <tbody>
                            @foreach ($spptdetail as $item)
                                <tr
                                    ...
                                    <!-- Number -->
                                    <td class="px-4 py-3 font-semibold">{{ $item->sppt_no }}</td>
                                    ...
                                    @foreach ($spptdetail as $row)
                                        <tr data-row-id="{{ $row->id }}" data-cpny="{{ $row->budget_cpny_id }}"
                                            ...
                                            <td>{{ $row->inventory_descr }}<br>
                                                <span class="text-sm text-gray-500 dark:text-gray-400">Note :
                                                    {{ $row->note }}</span><br>
                        ...
                        <div class="max-h-[60vh] overflow-y-auto p-4">
                            <table class="w-full text-sm">
                                ...
                                <tbody id="coaPickerTbody">
                                    <tr>
                                        <td colspan="7" class="p-4 text-center italic text-gray-500 dark:text-gray-400">Pilih filter
                                            lalu Apply</td>
                                    </tr>
                                </tbody>
                            </table>
    ...
    @include('partials.private-note-widget', ['doctype' => 'PT', 'refnbr' => $sppt->spptid])
    ...
    <script>
        $(document).ready(function() {
            ...
            attachMentionAutocomplete({
                ...
                fetchUrlFn: () => `/mentionable-users/${doctype}/${spptid}`,
            });
    ...
    <script>
        function closeOrRedirect(fallbackUrl = '/sppts') {
    ...
    <script>
        $(function() {
            ...
            $(document).on('click', '#btnSaveEditCoa', function() {
                ...
                $.ajax({
                    url: "{{ route('coa.update', $sppt->spptid ?? ($sppt->id ?? null)) }}",
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
