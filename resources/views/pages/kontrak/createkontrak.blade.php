<x-app-layout>
        ...
        <div class="mb-4 flex items-center justify-between">
            <div>
                <button onclick="history.back()"
                    class="inline-flex items-center gap-1 rounded-md bg-gray-100 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-700/30 dark:text-gray-300 dark:hover:bg-gray-600/50">
            ...
            <div class="flex w-full items-stretch gap-6 xl:flex-row">
                ...
                <div class="flex flex-1 flex-col rounded-xl bg-white dark:bg-gray-800">
                    ...
                    <div class="flex flex-1 flex-col overflow-y-auto px-4 py-2">
                        @php
                            $fields = [
                                ['icon'=>'calendar-days','label'=>'Kontrak Date','value'=> optional($kontrak->kontrakdate)->format('d M Y') ?? ($kontrak->kontrakdate ?? '-'),'is_raw'=>false],
                ...
                <div class="flex flex-1 flex-col rounded-xl bg-white dark:bg-gray-800">
                    <div class="p-4">
                        <form id="kontrakForm" class="space-y-4" @submit.prevent>
                            ...
                            {{-- Note --}}
                            <div>
                                <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Kontrak Note</label>
                                @if($isHold)
                                ...
                                @else
                                    <div class="rounded-md border border-gray-200 bg-white p-2 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 whitespace-pre-wrap">
                                        {{ $kontrak->kontaknote ?? '-' }}
                                    </div>
                                @endif
                            </div>
                    ...
                    <div x-data="{ activeTab: 'attachment' }" class="flex flex-1 flex-col border-t border-gray-200 dark:border-gray-700">
                        <header class="flex items-center rounded-t-none border-b border-gray-200 bg-gray-50 px-6 py-2 dark:border-gray-700 dark:bg-gray-700">
                        ...
                        <div x-show="activeTab === 'attachment'" class="flex h-full flex-1 flex-col transition-all">
                            ...
                            @if($isHold)
                                <form id="kontrakAttachmentUploadForm" enctype="multipart/form-data"
                                    ...
                                    <input type="hidden" name="departementid" value="{{ $kontrak->department_id }}">
                                    ...
                                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:gap-4">
                                        <div class="flex-1">
                                            ...
                                            <div class="flex items-center gap-3">
                                                <input type="file" id="kontrakAttachFiles" name="attachments[]" multiple
                                            ...
                                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Max 10 files, PDF / Image preferred.</p>
                                        </div>
                                    </div>

                                    <div id="kontrakUploadProgress" class="mt-4 hidden">
                                        <div class="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                                            <div id="kontrakUploadBar" class="h-2 w-0 rounded-full bg-indigo-600 transition-all duration-300 ease-out dark:bg-indigo-500"></div>
                                        </div>
                                        <p id="kontrakUploadPct" class="mt-1 text-sm text-gray-600 dark:text-gray-300">0%</p>
                                    </div>
                                </form>
                            @endif
                        </div>
    ...
    <script>
        ...
        $(function() {
            ...
            function val(id){ return ($('#'+id).val() ?? '').toString().trim(); }

            function validateKontrakForm() {
                clearMarks();
                const errs = [];

                if (!val('kontraktype')) errs.push({id:'kontraktype', msg:'Kontrak Type wajib dipilih.'});
                if (!val('kontrakcategory')) errs.push({id:'kontrakcategory', msg:'Kontrak Category wajib dipilih.'});
                if (!val('kontrakdate')) errs.push({id:'kontrakdate', msg:'Kontrak Date wajib diisi.'});
                if (!val('startdate')) errs.push({id:'startdate', msg:'Start Date wajib diisi.'});
                if (!val('enddate')) errs.push({id:'enddate', msg:'End Date wajib diisi.'});

                const sd = val('startdate');
                const ed = val('enddate');
                if (sd && ed && ed < sd) errs.push({id:'enddate', msg:'End Date harus >= Start Date.'});

                if (errs.length) {
                    errs.forEach(e => markInvalid($('#'+e.id)));
                    const first = errs[0];
                    $('#'+first.id).focus()[0]?.scrollIntoView({behavior:'smooth', block:'center'});
            ...
            $('#submitKontrakBtn').on('click', function(e){
                ...
                $.ajax({
                    url: @json(route('kontrak.submit', ['kontrakid' => '__KID__'])).replace('__KID__', encodeURIComponent(kontrakid)),
                    type: 'POST',
        ...
        $(document).ready(function() {
            ...
            function addComment() {
                ...
                $.ajax({
                    url: `/comments/${doctype}/${refnbr}`,
                    type: 'POST',
                    data: { comment: input, _token: '{{ csrf_token() }}' },
                    success: function(response) {
    ...
    <script>
        ...
        $(function() {
            ...
            function $tbody() { return $('#kontrakAttachmentTbody'); }
            ...
            $('#btnResetKontrakAttachment').on('click', function() {
                try { $('#kontrakAttachFiles')[0].value = ''; } catch(e){}
                const $fresh = $('#kontrakAttachFiles').clone({ withDataAndEvents:false });
