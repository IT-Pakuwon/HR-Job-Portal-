...
<div class="content-wrapper" style="background-color: #051931;">
    ...
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li class="active" style="color: white">Warehouse Dept</li>
        </ol>
    </section>
    ...
    <section class="content" style="margin-top:2rem">
        <div class="box box-info" style=" border-top:none; border-radius:10px;"> 
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <h1 style="color: Black; padding-left:2rem; font-size:2rem;">Warehouse Dept</h1>
                <div>
                <a class="btn" style="background-color:#006EF0;color:white;margin-right: 10px"  href="javascript:void(0)" id="createNewMswhs"> New Whs Dept</a>
                </div>
            </div>      
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="inputForm" name="inputForm" class="form-horizontal">
                                ...
                                <div class="form-group">
                                    <div class="col-md-12">
                                        <label for="cpnyid" class="col-sm-3 control-label">Departement</label>
                                        <div class="col-sm-9">
                                            <select class="form-control select2" id="deptx" name="dept" style="width: 100%;" required>
                                                <option selected="selected"></option>
                                                @foreach($dept as $p)
    ...
    $(function () {   
      ...
      var table = $('.data-table').DataTable({
        ...
        $('#createNewMswhs').click(function () {
            $('#saveBUTTON').html("Save Whs Dept");
            $('#key_id').val('');
            $('#inputForm').trigger("reset");
            $('#modelHeading').html("Create New Whs Dept");
            $('#ajaxModel').modal('show');
            $('#cpnyidx').val(null).trigger('change');
            $('#deptx').val(null).trigger('change');
            $('#whs_typex').val(null).trigger('change');
            $('#statusx').val(null).trigger('change');
        });        
        
        $('body').on('click', '.editMswhsdept', function () {
        var key_id = $(this).data('id');
        $.get("{{ route('mswhsdept.mswhsdept') }}" +'/' + key_id +'/edit', function (data) {
            $('#modelHeading').html("Edit Whs Dept");
            $('#saveBUTTON').html("Updated Whs Dept");
            $('#ajaxModel').modal('show');
            $('#key_id').val(data.id);
            $('#whs_id').val(data.whs_id);       
            $('#cpnyidx').val(data.cpnyid).trigger('change'); 
            $('#deptx').val(data.department_id).trigger('change');
        ...
        $('#saveBUTTON').click(function (e) {
            ...
            url: "{{ route('mswhsdept.save_whsdept') }}",
            type: "POST",
            dataType: 'json',
