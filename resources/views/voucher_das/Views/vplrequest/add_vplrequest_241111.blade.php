...
<div class="content-wrapper" style="background-color: #051931; border-radius: 5px 0 0 0; margin-top:3rem; padding-left:2rem; padding-right:2rem">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb" style="background: transparent;">
            ...
            <li class="active" style="color: white;font-size:13px">Create Request</li>
        </ol>
    </section>
        ...
        <div class="box box-info" style=" border-radius:5px; background-color:white; border-top:none; ">
            ...
            <form id="inputForm" name="inputForm" class="form-horizontal" enctype="multipart/form-data">
                <div class="box-body">
                        ...
                        <div class="col-md-12">                            
                            ...
                            <div class="col-sm-3">
                                <select class="form-control select2" id="requesttype" name="requesttype" style="width: 100%;" >
                                    ...
                                    <option value="ReturnRq">Return</option>
                    ...
                    <hr>
                    <div class="box-body">
                        <h5 class="modal-title">Request Details</h5><br>
                        <table class="table table-bordered" id="dynamicTable">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 15%;"><span style="color:red;font-weight:bold"> *</span>From WHS</th>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><input type="text" name="addmore[0][qty_request]" placeholder="Enter Qty" class="form-control" value="0"  /></td>   
...
<script type="text/javascript">
    ...
    function validateForm() {
        ...
        isValid &= validateSelectField('requesttype', 'Source of Request is required');        
   ...
   // Validation logic for the dynamic table
    function validateDynamicTable() {
        ...
        $('#dynamicTable tbody tr').each(function() {           
            ...
            var qtyRequestField = $(this).find('input[name^="addmore"]').filter('[name*="[qty_request]"]');
            
            var productName = productNameField.val();      
            var qtyAvailable = parseFloat(qtyAvailableField.val()) || 0;
            var qtyRequest = parseFloat(qtyRequestField.val()) || 0
            ...
            if (!qtyRequest || qtyRequest <= 0) {
                qtyRequestField.after('<span class="error-message" style="color: red;">Qty Request is required and must be greater than 0</span>');
                isValid = false;
            } else if (qtyRequest > qtyAvailable) {
                qtyRequestField.after('<span class="error-message" style="color: red;">Qty Request cannot be greater than available stock</span>');
                isValid = false;
            }
...
<script>
    ...
    $('#addRow').click(function() {
        ...
        $.ajax({
            url: "{{ route('getProductsByRequestType') }}", // Buat route untuk handle request ini
            type: 'GET',
            data: { requesttype: requesttype },
            success: function(data) {
                // Update tabel modal dengan data yang diterima
                var tableBody = $(`#example_${i} tbody`);
                tableBody.empty(); // Kosongkan tabel

                // Loop data dan append ke dalam tabel
                        ...
                        <tr>
                            ...
                            <td>
                                <button type="button" class="btn btn-primary select-product"
                                    ...
                                    data-row-index="${i}" data-dismiss="modal">Select</button>
                            </td>
                        </tr>
                    `);
                });
        ...
        // Initialize DataTables after adding the modal
        $(`#example_${i}`).DataTable();
       console.log(i);
    });
    ...
    $(document).on('click', '.select-product', function () {
        ...
        console.log('Selected Product:', productName,'ID:', productId,'qty:', qty,'expiredDate:', expiredDate,'whsId:', whsId, 'For Row Index:', rowIndex);

        // Insert the selected product data into the appropriate fields
        $(`#product_id_${rowIndex}`).val(productId);  // hidden product_id field
        $(`#product_name_${rowIndex}`).val(productName); // product name field
        $(`#qty_available_${rowIndex}`).val(qty);  // quantity field
        $(`#expired_date_${rowIndex}`).val(expiredDate); // expired date field
        $(`#from_whs_id_${rowIndex}`).val(whsId); // warehouse ID field
    });
...
$('#requesttype').on('change', function() {
    var requesttype = $(this).val(); // Ambil nilai request type (Request atau Return)
    var i =0;
    // Ajax request untuk mengambil data produk berdasarkan request type
    $.ajax({
        url: "{{ route('getProductsByRequestType') }}", // Buat route untuk handle request ini
        type: 'GET',
        data: { requesttype: requesttype },
