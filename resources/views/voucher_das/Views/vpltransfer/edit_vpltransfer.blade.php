...
<div class="content-wrapper" style="background-color: #051931; border-radius: 5px 0 0 0; margin-top:3rem; padding-left:2rem; padding-right:2rem">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb" style="background: transparent;">
            ...
            <li class="active" style="color: white;font-size:13px">Edit Transfer</li>
        </ol>
    </section>
                ...
                <div class="box-body">
                    <div class="box-body">
                        <div class="col-md-12">                            
                            ...
                            <div class="col-sm-2">
                                ...
                                <input type="hidden" name="cpnyid" value="{{ $vpltransfer->cpnyid }}">          
                            </div>     
                            ...
                            <div class="col-sm-2">
                                ...
                                <input type="hidden" name="department" value="{{ $vpltransfer->department }}">                                                 
                            </div> 
                            ...
                            <div class="col-sm-2">
                                <select class="form-control select2" id="vp_type_display" name="vp_type_display" style="width: 100%;" required disabled>
                                        <option value="" {{ $vpltransfer->vp_type == '' ? 'selected' : '' }}></option>
                                        <option value="Voucher" {{ $vpltransfer->vp_type == 'V' ? 'selected' : '' }}>Voucher</option>
                                        <option value="Product" {{ $vpltransfer->vp_type == 'P' ? 'selected' : '' }}>Product</option>
                                </select> 
                                <input type="hidden" name="vp_type" value="{{ $vpltransfer->vp_type }}">
                            </div> 
                    ...
                    <div class="box-body">
                        <div class="col-md-12">  
                            ...
                            <div class="col-sm-2">
                                <select class="form-control select2" id="transfertype_display" name="transfertype_display" style="width: 100%;" disabled>                                   
                                    ...
                                    <option value="ReturnTf" {{ $vpltransfer->transfertype == 'ReturnTf' ? 'selected' : '' }}>Return</option>
                                </select>                              
                                <input type="hidden" name="transfertype" value="{{ $vpltransfer->transfertype }}">
                            </div>     
                            <label for="inputEmail3" class="col-sm-2 control-label" ><span style="color:red;font-weight:bold"> *</span>Reference</label>                            
                            <div class="col-sm-2">
                                <select class="form-control select2" id="ref_transfer_id_display" name="ref_transfer_id_display" id="ref_transfer_id_display" style="width: 100%;" disabled>                                    
                                    @foreach($vpltransfer_completed as $p)
                                        <option value="{{ $p->ref_transfer_id }}"
                                            {{ $p->ref_transfer_id == $vpltransfer->ref_transfer_id ? 'selected' : '' }}>
                                            {{ $p->ref_transfer_id }}
                                        </option>
                                    @endforeach
                                </select>      
                                <input type="hidden" id="ref_transfer_id" name="ref_transfer_id" value="{{ $vpltransfer->ref_transfer_id }}">                    
                            </div>   
                        </div>
                    </div>
                    ...
                    <div class="box-body">
                        ...
                        <table class="table no-margin">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 10%;">From Gudang</th>  
                                    <th style="width: 10%;">To Gudang</th>     
                        ...
                        <table class="table table-bordered" id="dynamicTable">
                            <thead>
                                <tr>
                                    {{-- <th style="width: 15%;"></span>From WHS</th>
                                    <th style="width: 40%;"></span>Product Name</th>
                                    <th style="width: 8%;"></span>Stock</th>                                
                                    <th style="width: 10%;"></span>Expired Date</th>                                    
                                    <th style="width: 10%;"></span>Qty</th>
                                    <th style="width: 15%;"></span>To WHS</th>
                                    <th style="width: 5%;">Action</th> --}}
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td>
                                        {{-- <select class="form-control select2" id="addmore[0][from_whs_id]" name="addmore[0][from_whs_id]" style="width: 100%;">                                       
                                        </select> --}}
...
<script>
    $(function () {
    const val = $('#ref_transfer_id').val();      // nilai dari hidden
    const $sel = $('#ref_transfer_id_display');   // select2 yang tampil

    // init select2 (kalau belum)
    if (!$sel.data('select2')) $sel.select2({ width: '100%' });

    if (val) {
        // kalau option dengan value tsb belum ada, tambahkan lalu pilih
    ...
    // kalau nanti select2 di-enable & diubah user, sync balik ke hidden
    $sel.on('change', function () {
        $('#ref_transfer_id').val(this.value);
    });
    });
</script>
   ...
   // Validation logic for the dynamic table
    function validateDynamicTable() {
        ...
        $('#dynamicTable tbody tr').each(function() {   
            ...
            console.log(qtyAvailable,'-',qtyTransfer);
            if (qtyAvailable !== 0) {
                    // Validate qty_transfer field
...
<script>
    $(document).ready(function() {
        ...
        var transfertype= $('input[name="transfertype"]').val(); // 'Transfer' / 'ReturnTf'
        var refTtransfer= $('input[name="ref_transfer_id"]').val();
        var warehouseId = $('#from_whs_id_0').val();
        ...
        function loadProducts(cpnyid,vp_type,transfertype,warehouseId, refTtransfer,rowIndex) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    const isTransfer = (transfertype === 'Transfer');
        ...
        $('select[name="cpnyid"]').on('change', function() {          
            ...
            var department = $('select[name="department"]').val(); // Ambil nilai cpnyid
            var vp_type = $('select[name="vp_type"]').val(); // Ambil nilai cpnyid
            var refTtransfer = $('input[name="ref_transfer_id"]').val();
        ...
        $('#ref_transfer_id').on('change', function() {
            ...
            if (refTtransfer) {
                console.log("Fetching From WHS for:", cpnyid, vp_type, refTtransfer);
        ...
        $('#addRow').click(function() {
            ...
            const cpnyid       = $('input[name="cpnyid"]').val()       || $('select[name="cpnyid"]').val();
            const department   = $('input[name="department"]').val()   || $('select[name="department"]').val();
            const vp_type      = $('input[name="vp_type"]').val()      || $('select[name="vp_type"]').val();
            const transfertype = $('input[name="transfertype"]').val() || $('#transfertype').val();
            const refTtransfer = $('input[name="ref_transfer_id"]').val() || $('#ref_transfer_id').val();

            // if (i > 0) {
            //     // Pastikan refTtransfer sama dengan row pertama
            //     refTtransfer = $('input[name="ref_transfer_id"]').val();
            // }

            // loadToWhsOptions(cpnyid, transfertype,i);
            // loadFromWhsOptions(cpnyid,transfertype,i);
            // loadProducts(cpnyid,transfertype,warehouseId, i);
        ...
        function reloadDataTable(rowIndex, data) {
            // const $tbody = $(`#example_${rowIndex} tbody`);            
            const $tbl   = $(`#example_${rowIndex}`);
            const $thead = $tbl.find('thead tr');
