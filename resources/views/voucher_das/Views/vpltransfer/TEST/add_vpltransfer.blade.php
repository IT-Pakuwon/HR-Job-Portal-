...
<script>
    $(document).ready(function() {
        ...
        console.log(cpnyid,'-',department,'-',transfertype);
        loadFromWhsOptions(cpnyid,department,transfertype,0);
        ...
        $('#transfertype').on('change', function() {
            ...
            if (transfertype) {
                console.log("Fetching From WHS for:", cpnyid, department, transfertype);

                // **Pastikan warehouseId diperbarui sebelum memanggil loadProducts**
                loadFromWhsOptions(cpnyid, department, transfertype, 0, function(warehouseId) {
                    console.log("Using updated warehouseId:", warehouseId);
                    loadProducts(cpnyid, transfertype, warehouseId,refTtransfer, 0);
                    loadToWhsOptions(cpnyid, department, transfertype,warehouseId,0);
                });
            }
        ...
        $('#ref_transfer_id').on('change', function() {
            ...
            if (refTtransfer) {
                console.log("Fetching From WHS for:", cpnyid, department, refTtransfer);
        ...
        function loadToWhsOptions(cpnyid, department, transfertype, warehouseId,rowIndex) {
            // Pastikan cpnyid dan transfertype tidak kosong
            if (!cpnyid || !department || !transfertype) {
                return;
            }
        ...
        function loadFromWhsOptions(cpnyid, department, transfertype, rowIndex, callback) {
            if (!cpnyid || !department || !transfertype ) {
                console.warn("Skipping loadFromWhsOptions: Missing cpnyid or department or transfertype");
                return;
            }
        ...
        $('#addRow').click(function() {
            ...
            loadFromWhsOptions(cpnyid, department, transfertype,i, function(warehouseId) {
            loadProducts(cpnyid, transfertype, warehouseId,refTtransfer, i);
            loadToWhsOptions(cpnyid, department,transfertype,warehouseId,i);
            });
        });
