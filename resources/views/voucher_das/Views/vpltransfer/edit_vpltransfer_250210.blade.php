...
<script>
    $(document).ready(function() {
        ...
        function loadProducts(cpnyid,transfertype,warehouseId, rowIndex) {
            $.ajax({
        ...
        $('#transfertype').on('change', function() {
            var transfertype = $('#transfertype').val(); // Ambil nilai transfertype
            var cpnyid = $('select[name="cpnyid"]').val(); // Ambil nilai cpnyid            
            // loadToWhsOptions(cpnyid,transfertype,0);
            loadFromWhsOptions(cpnyid,transfertype,0);
        });

        $(document).on('change', 'select[name^="addmore"][name*="[from_whs_id]"]', function () {
            var warehouseId = $(this).val();           
            // var rowIndex = $(this).closest('tr').attr('id').replace('row', '');
            var row = $(this).closest('tr');
            if (row.length > 0 && row.attr('id')) {
                var rowIndex = row.attr('id').replace('row', '');
                console.log('Row Index:', rowIndex);
            } else {
                rowIndex = 0;
                // console.error('Row not found or missing id attribute');
            }
            var cpnyid = $('select[name="cpnyid"]').val();
            console.log('Changed WHS:', warehouseId);
            loadProducts(cpnyid, transfertype,warehouseId,rowIndex); // Pastikan fungsi ini benar
            loadToWhsOptions(cpnyid,transfertype,warehouseId,rowIndex);
        });
        ...
        function loadFromWhsOptions(cpnyid, transfertype, rowIndex) {
            // Pastikan cpnyid dan transfertype tidak kosong
            if (!cpnyid) {
                return;
            }
            ...
            $.ajax({
                url: "{{ route('getFromWhsOptionsTransfer') }}", // Route ke controller untuk query Mswhsdept
                type: "GET",
                data: {
                    cpnyid: cpnyid                                   
                },
                success: function (response) {
                    // Perbarui opsi di select to_whs_id untuk baris yang sesuai
                    var toWhsSelect = $(`select[name='addmore[${rowIndex}][from_whs_id]']`);
                    toWhsSelect.empty(); // Kosongkan opsi sebelumnya
