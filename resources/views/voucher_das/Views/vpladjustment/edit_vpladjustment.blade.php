...
<div class="content-wrapper" style="background-color: #051931; border-radius: 5px 0 0 0; margin-top:3rem; padding-left:2rem; padding-right:2rem">
    ...
    <section class="content-header">
        <h1 style="color: white">Adjustment No : {{ $vpladjustment->adjustment_id }}</h1>
        <ol class="breadcrumb" style="background: transparent;">
            <li><a href="#" style="color: white;font-size:13px"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active" style="color: white;font-size:13px">Edit Adjustment</li>
        </ol>
    </section>
        ...
        <div class="box box-info" style=" border-radius:5px; background-color:white; border-top:none; ">          
            <form id="inputForm" name="inputForm" class="form-horizontal" enctype="multipart/form-data">
                <div class="box-body">
                    <div class="box-body">
                        <div class="col-md-12">                            
                            ...
                            <div class="col-sm-2">
                                <input type="hidden" class="form-control" name='idx'value="{{ $vpladjustment->id }}">
                                <select class="form-control select2" name="cpnyid" style="width: 100%;" required>                                    
                                    @foreach($usercpny as $p)
                                        <option value="{{ $p->cpnyid }}" {{ $p->cpnyid == $vpladjustment->cpnyid ? 'selected' : '' }}>{{ $p->cpnyid }}</option>
                                    @endforeach
                                </select>                               
                            </div>     
                            ...
                            <div class="col-sm-3">
                                <select class="form-control select2" name="department" style="width: 100%;" required>                                    
                                    ...
                                    <option value="{{ $p->deptname }}" {{ $p->deptname == $vpladjustment->department ? 'selected' : '' }}>{{ $p->deptname }}</option>
                                @endforeach
                                </select>                               
                            </div>                              
                    ...
                    <div class="box-body">
                        <div class="col-md-12">  
                            ...
                            <div class="col-sm-4">
                                <textarea type="text" rows="3" class="form-control" name="adjustment_remark"  placeholder="Enter Remarks ...">{{ $vpladjustment->adjustment_remark }}</textarea>
                            </div>
                        </div>
                    </div>
                    <hr>
                    <div class="box-body">
                        <h5 class="modal-title">Adjustment Details</h5><br>
                        <table class="table no-margin">
                            <thead>
                                <tr>
                                    <th style="width: 15%;">ProductID</th> 
                                    <th style="width: 15%;">Name</th>                                    
                                    <th style="width: 10%;">Qty Adjustment</th>                                      
                        ...
                        <table class="table table-bordered" id="dynamicTable">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 15%;"></span>Gudang</th>
                                    <th style="width: 10%;"></span>Qty Adjustment</th>                                  
                                    <th style="width: 5%;">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    ...
                                    <td><input type="text" name="addmore[0][from_whs_id]" placeholder="Enter Whs" class="form-control" id="from_whs_id_0" readonly /></td>   
                                    <td><input type="text" name="addmore[0][qty_adjustment]" placeholder="Enter Qty" class="form-control" value="0"  /></td>                                 
                ...
                <div class="box-footer">
                    ...
                    <a href="{{ url('/vpladjustment_waiting') }}" style="margin-right: 10px;background-color:#EA002F; color:white" class="btn mb-1">Cancel</a>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#saveBUTTON').click(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('updatevpladjustment') }}", // Ubah dengan route yang sesuai
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
                    ...
                    // setTimeout(function() {
                        window.location.href = "{{ route('vpladjustment_waiting.vpladjustment_waiting') }}";
   ...
   // Validation logic for the dynamic table
    function validateDynamicTable() {
        ...
        $('#dynamicTable tbody tr').each(function() {   
            ...
            var qtyAdjustmentField = $(this).find('input[name^="addmore"]').filter('[name*="[qty_adjustment]"]');
            
            var qtyAvailable = parseFloat(qtyAvailableField.val()) || 0;
            var qtyAdjustment = parseFloat(qtyAdjustmentField.val()) || 0;
            console.log(qtyAvailable,'-',qtyAdjustment);
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            if (confirm('Are you sure you want to delete this detail?')) {
                ...
                $.ajax({
                    url: '{{ route("delete_vpladjustment_detail") }}', // Update with your route
                    type: 'POST',
                    data: {
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.delete-btn-attachment', function() {
            ...
            if (confirm('Are you sure you want to delete this attachment?')) {
                // AJAX request to delete the record
                $.ajax({
                    url: '{{ route("delete_vpladjustment_attach") }}', // Update with your route
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}', // CSRF protection
                        detail_id: detailId
...
<script>
    $(document).ready(function() {
        ...
        // Fungsi umum untuk memuat data produk berdasarkan cpnyid dan adjustmenttype
        function loadProducts(cpnyid, rowIndex) {
            $.ajax({
                url: "{{ route('getProductsByAdjustmentType') }}",
                type: "GET",
                data: {                  
                    cpnyid: cpnyid
                },
        ...
        // Panggilan awal saat halaman pertama kali di-load       
        var initialCpnyid = $('select[name="cpnyid"]').val();
        
        // Load untuk row pertama (index 0)
        loadProducts(initialCpnyid, 0);
        loadToWhsOptions(initialCpnyid, 0);
     
        // Event listener saat cpnyid atau adjustmenttype berubah
        $('select[name="cpnyid"]').on('change', function() {           
            var cpnyid = $('select[name="cpnyid"]').val(); // Ambil nilai cpnyid
            loadProducts(cpnyid, 0); // Load data untuk modal awal (rowIndex 0)
            loadToWhsOptions(cpnyid,0);           
                       
        });

       

        function loadToWhsOptions(cpnyid,rowIndex) {
            // Pastikan cpnyid dan adjustmenttype tidak kosong
            if (!cpnyid) {
                return;
            }

            $.ajax({
                url: "{{ route('getToWhsOptionsAdjustment') }}", // Route ke controller untuk query Mswhsdept
                type: "GET",
                data: {
                    cpnyid: cpnyid                               
                },
        ...
        $('#addRow').click(function() {
            ...
            loadProducts(cpnyid, i);
            loadToWhsOptions(cpnyid, i);
        });
