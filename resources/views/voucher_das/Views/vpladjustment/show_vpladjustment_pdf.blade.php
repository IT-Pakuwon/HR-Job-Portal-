...
<table style="height: 58px; width: 684px;">
    ...
    <tr>
        <td style="width: 674px; text-align: center;">
            <h3><strong>ADJUSTMENT VOUCHER/PRODUCT</strong></h3>
        </td>
    </tr>   
    </tbody>
</table>
    ...
    <tbody>        
        {{-- <tr style="height: 21px;">
            <td style="width: 160.25px; height: 21px;">Type Adjustment</td>
            <td style="width: 500.75px; height: 21px;">: {{ $adjustmenttype }}</td>
        </tr>             --}}
        <tr style="height: 25px;">
            <td style="width: 160.25px; height: 25px;">Remark</td>
            <td style="width: 500.75px; height: 25px;">: {{ $adjustment_remark }}</td>
        </tr>
    </tbody>
</table>  
{{-- <p>&nbsp;</p> --}}
...
<table border="1" cellpadding="1" cellspacing="1" style="width:720px; border-collapse: collapse; border: 1px solid black;">
    <tbody>
        ...
        @foreach($vpladjustmentdetail as $dt)    
        <tr style="height: 26px;">
            <td style="width: 80px; height: 26px;">{{ $dt->product_id }}</td>
            <td style="width: 100px; height: 26px;">{{ $dt->expired_date == '1900-01-01' ? 'No Expired' : $dt->expired_date }}</td>
            <td style="width: 150px; height: 26px;">{{ $dt->product_name }}</td>            
            <td style="width: 30px; height: 26px;">{{ $dt->qty_adjustment }}</td>
