...
<table style="height: 58px; width: 684px;">
    ...
    <tr>
        <td style="width: 674px; text-align: center;">
            <h3><strong>RECEIVE VOUCHER/PRODUCT</strong></h3>
        </td>
    </tr>   
    </tbody>
</table>
    ...
    <tbody>
        <tr style="height: 21px;">
            <td style="width: 160.25px; height: 21px;">Tenant Name</td>
            <td style="width: 500.75px; height: 21px;">: {{ $receive_tenant }}</td>
        </tr>
        <tr style="height: 21px;">
            <td style="width: 160.25px; height: 21px;">Type Receive</td>
            <td style="width: 500.75px; height: 21px;">: {{ $receive_type }}</td>
        </tr>
        <tr style="height: 23px;">
            <td style="width: 160.25px; height: 23px;">Source Department</td>
            <td style="width: 500.75px; height: 23px;">: {{ $source_receive_dept }}</td>
        </tr>       
        <tr style="height: 25px;">
            <td style="width: 160.25px; height: 25px;">Remark</td>
            <td style="width: 500.75px; height: 25px;">: {{ $receive_remark }}</td>
        </tr>
    </tbody>
</table>  
<p>&nbsp;</p>
...
<table border="1" cellpadding="1" cellspacing="1" style="width:720px; border-collapse: collapse; border: 1px solid black;">
    <tbody>
        <tr style="height: 21px;">
            <td style="width: 50px; height: 21px; text-align: center;">ProductID</td>
            <td style="width: 75px; height: 21px; text-align: center;">Expired Date</td>
            <td style="width: 300px; height: 21px; text-align: center;">Name</td>
            <td style="width: 50px; height: 21px; text-align: center;">Qty</td>
        </tr>
        @foreach($vplreceivedetail as $dt)    
        <tr style="height: 26px;">
            <td style="width: 50px; height: 26px; text-align: center;">{{ $dt->product_id }}</td>
            <td style="width: 75px; height: 26px; text-align: center;">
                {{ $dt->expired_date == '1900-01-01' ? 'No Expired' : $dt->expired_date }}
            </td>
            <td style="width: 300px; height: 26px;">{{ $dt->product_name }}</td>
            <td style="width: 50px; height: 26px; text-align: center;">{{ $dt->qty_receive }}</td>
        </tr>      
        @endforeach    
    </tbody>
</table>
