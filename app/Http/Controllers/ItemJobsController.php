...
class ItemJobsController extends Controller
{
    ...
    {
        $request->validate([
            'inventoryid'     => ['required','string','max:50', Rule::unique('pgsql.ms_inventory','inventoryid')],
        ...
        try {
            ...
            $inv = MsInventory::create([
                ...
                // default item_type kalau kosong
                'item_type'       => 'GI',
                'item_sub_type'   => strtoupper($request->item_sub_type),
                'item_class'      => strtoupper($request->item_class),
                'item_sub_class'  => strtoupper($request->item_sub_class),
            ...
            DB::rollBack();
            return response()->json(['error' => 'Gagal menyimpan inventory', 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'inventoryid'     => ['required','string','max:50', Rule::unique('pgsql.ms_inventory','inventoryid')->ignore($inv->id)],
            'inventory_descr' => ['required','string','max:255'],
            'item_type'       => ['nullable','string','max:100'],
            'item_sub_type'   => ['nullable','string','max:100'],
            'item_class'      => ['nullable','string','max:100'],
        ...
        try {
            ...
            DB::rollBack();
            return response()->json(['error' => 'Gagal update inventory', 'message' => $e->getMessage()], 500);
        }
    }
