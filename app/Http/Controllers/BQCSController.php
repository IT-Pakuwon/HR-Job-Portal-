...
class BQCSController extends Controller
{
    ...
    public function storeBQCS(Request $request)
    {
        ...
        try {
            // ambil type dari CS
            $bqType = trim((string)($cs->bqtype ?? ''));
            if ($bqType === '') {
                $bqType = 'Kontrak'; // fallback default
            }
            ...
            // mapping vendor ke kolom vendorid1..6 + grand total
            ...
            foreach ($details as $row) {
                ...
                // simpan type sesuai CS
                $det->bqtype = $bqType;

                // hanya simpan duration untuk kontrak
                $det->kontrak_duration_qty = strtoupper($bqType) === 'KONTRAK' ? $duration : null;
                ...
                for ($i = 0; $i < min(count($row['vendor'] ?? []), 6); $i++) {
                    ...
                    // jika Jasa => tidak kali duration
                    // jika Kontrak => kali duration
                    if (strtoupper($bqType) === 'JASA') {
            ...
            // update header grand total material/jasa
            for ($i = 1; $i <= 6; $i++) {
    ...
    public function updateBQCS(Request $request, $hash)
    {
        ...
        return DB::connection('pgsql')->transaction(function () use ($id, $vendors, $details, $username, $now) {
            ...
            // =========================
            // 0) Ambil bqtype dari header
            // =========================
            $type = strtoupper(trim((string)($hdr->bqtype ?? 'KONTRAK')));
            if (!in_array($type, ['JASA', 'KONTRAK'], true)) {
                $type = 'KONTRAK';
            }
            ...
            foreach ($details as $row) {
                ...
                // samakan dengan store: type ikut header
                $det->bqtype = $type;

                // JASA: tidak perlu duration
                // KONTRAK: simpan duration
                ...
                // set ulang vendorid1..6 + total per vendor
                for ($i = 1; $i <= 6; $i++) {
    ...
    public function updateBQCS_zzz(Request $request, $hash)
    {
        $id = Hashids::decode($hash)[0] ?? null;
        abort_if(!$id, 404);
