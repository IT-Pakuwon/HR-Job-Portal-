...
class VplReceiveController extends Controller
{
    ...
    const DOCTYPE     = 'VPR';
    const DOCTYPE_DSC = 'Voucher Product Receive';
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', fn ($r) =>
                    '<button type="button" class="btn-view inline-flex items-center rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-indigo-600" data-id="' . $r->id . '">' . $r->receive_id . '</button>'
                )
    ...
    public function showData(int $id)
    {
        ...
        if (!$receive) return response()->json(['error' => 'Not found'], 404);
    ...
    public function update(Request $request, int $id)
    {
        ...
        try {
            if ($request->has('addmore')) {
                foreach ($request->addmore as $detail) {
                    if (empty($detail['product_name']) || empty($detail['qty']) || empty($detail['whs_id'])) continue;
                    $exp = !empty($detail['expired_date']) ? $detail['expired_date'] : '1900-01-01';
    ...
    public function deleteDetail(Request $request)
    {
        ...
        if (!$detail) return response()->json(['error' => 'Not found.'], 404);
        $detail->delete();
    ...
    public function deleteAttachment(Request $request)
    {
        ...
        if (!$attach) return response()->json(['error' => 'Not found.'], 404);
        $attach->delete();
    ...
    public function getProducts(Request $request)
    {
        ...
        $products->transform(fn ($p) => tap($p, fn ($p) =>
            $p->product_label = $p->product_name . ' / ' . number_format($p->product_value, 0, '.', ',') . ' / ' . $p->product_uom
        ));
    ...
    {
        $base = 'w-32 pointer-events-none font-semibold px-4 py-2 text-center rounded text-xs';
        return match ($status) {
            'P'     => '<span class="' . $base . ' bg-orange-200/60 text-orange-800 border border-orange-600/40 dark:bg-orange-300/40 dark:text-orange-900">On Progress</span>',
            'C'     => '<span class="' . $base . ' bg-green-200/60 text-green-800 border border-green-600/40 dark:bg-green-300/40 dark:text-green-900">Completed</span>',
            'R'     => '<span class="' . $base . ' bg-red-200/60 text-red-800 border border-red-600/40 dark:bg-red-300/40 dark:text-red-900">Rejected</span>',
            'X'     => '<span class="' . $base . ' bg-red-200/60 text-red-800 border border-red-600/40 dark:bg-red-300/40 dark:text-red-900">Cancelled</span>',
            default => '<span class="' . $base . ' bg-amber-200/60 text-amber-800 border border-amber-600/40 dark:bg-amber-300/40 dark:text-amber-900">Hold / Revise</span>',
        };
    }
