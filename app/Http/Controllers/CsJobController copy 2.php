...
class CsJobController extends Controller
{
    ...
    public function CsJobs()
    {
        ...
        if ($this->revisionSourceForUser($u) === 'reusekontrak') {
            ...
            $revision = TrPOReuse::query()
                ->where('created_by', $u)
                ->whereRaw("UPPER(COALESCE(inventory_category,'')) = 'KONTRAK'")
                ->selectRaw('COUNT(DISTINCT ponbr) as total')
                ->value('total');
        } else {
            // $revision = vCsRevision::where('created_by', $u)->count();
            $revision = vCsRevision::query()
                ->where('created_by', $u)
                ->selectRaw('COUNT(DISTINCT ponbr) as total')
                ->value('total');
        }
    ...
    public function CsJobsDatasetCounts(Request $request)
    {
        ...
        return response()->json([
            'mine'      => vCsJobs::where('assignpurchasing', $u)->count(),
            // 'revision'  => vCsRevision::where('created_by', $u)->count(),
            // 'revision' => ($this->revisionSourceForUser($u) === 'reusekontrak')
                // ? TrPOReuse::query()
                //     ->where('created_by', $u)
                //     ->whereRaw("UPPER(COALESCE(inventory_category,'')) = 'KONTRAK'")
                //     ->count()
    ...
    public function CsJobsRevisionJson(Request $request)
    {
        ...
        return $this->buildVcsRevisionJson($request, $base);
    }

    public function CsJobsRevisionJson_zzz(Request $request)
    {
        $username = Auth::user()->username ?? '';

        if ($this->revisionSourceForUser($username) === 'reusekontrak') {
            $base = TrPOReuse::query()
    ...
    private function revisionSourceForUser(string $username): string
    {
        ...
        return $hasKontrakReuse ? 'reusekontrak' : 'reusepo';
    }
    ...
    private function buildReuseRevisionJson(Request $request, $base)
    {
        ...
        // total distinct ponbr sebelum search
        $recordsTotal = (clone $base)
            ->selectRaw('COUNT(DISTINCT ponbr) as total')
            ->value('total');

        // clone base untuk filter
        ...
        // total distinct ponbr setelah search
        $recordsFiltered = (clone $filteredBase)
            ->selectRaw('COUNT(DISTINCT ponbr) as total')
            ->value('total');

        // data dipaksa 1 row per ponbr
        $data = $filteredBase
            ->selectRaw("
                MIN(id) as id,
                ponbr,
                MAX(created_at) as created_at,
                MAX(cpny_id) as cpny_id,
                MAX(created_by) as created_by,
                MAX(csid) as csid,
                MAX(sppbjktid) as sppbjktid,
                MAX(inventory_category) as inventory_category,
                STRING_AGG(DISTINCT inventoryid::text, ', ') as inventoryid,
                STRING_AGG(DISTINCT inventory_descr::text, ', ') as inventory_descr
            ")
            ->groupBy('ponbr')
        ...
        $data->transform(function ($r) {
            ...
            if (strtoupper(trim((string) $r->inventory_category)) === 'KONTRAK') {
                $kontrak = TrKontrak::query()
                    ->select('id', 'kontrakid', 'department_id', 'vendorname')
                    ->where('kontrakid', $r->ponbr)
                    ->first();

                if ($kontrak) {
                    $r->kontrak_eid = Hashids::encode($kontrak->id);
                }
            }

            $r->podate         = $r->created_at;
            $r->department_id  = optional($kontrak)->department_id;
            $r->vendorname     = optional($kontrak)->vendorname;
    ...
    private function buildVcsRevisionJson(Request $request, $base)
    {
        $draw   = (int) $request->input('draw', 1);
        $start  = (int) $request->input('start', 0);
        ...
        $data = $filteredBase
            ...
            ")
            ->groupBy('ponbr')
            ->orderByRaw("MAX({$orderCol}) {$orderDir}")
            ->skip($start)
            ->take($length)
            ->get();
    ...
    private function buildReuseRevisionJson_xxx(Request $request, $base)
    {
        $draw   = (int) $request->input('draw', 1);
        $start  = (int) $request->input('start', 0);
        $length = (int) $request->input('length', 25);
        ...
        $data->transform(function ($r) {
            $r->doc_type = 'PO';
            $r->eid      = Hashids::encode($r->id);   // hash tr_po_reusekontrak (createcs)
            $r->doc_no   = $r->ponbr;
