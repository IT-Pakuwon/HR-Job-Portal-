...
class VplTransferController extends Controller
{
    ...
    public function showData(int $id)
    {
        ...
        $can_add_attachment = $transfer->created_user === $user->name && !in_array($transfer->status, ['X', 'R', 'D'], true);
    ...
    public function addAttachment(Request $request, int $id)
    {
        ...
        if ($transfer->created_user !== $user->name || in_array($transfer->status, ['X', 'R', 'D'], true)) {
            return response()->json(['error' => 'You are not allowed to modify this document.'], 403);
        }
        ...
        $this->saveAttachments($request, $transfer->transfer_id, Carbon::now()->year, $user);

        $attachments = Attachment::where('docid', $transfer->transfer_id)->where('status', 'A')->get();
