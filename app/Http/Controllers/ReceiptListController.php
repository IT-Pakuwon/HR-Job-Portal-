...
{
    private function returnAggSubquery()
    {
        // WAJIB: gunakan koneksi pgsql supaya tidak jadi SQL MySQL (backtick)
        return DB::connection('pgsql')
            ->table('tr_receipt_detail as d')
            ->join('tr_receipt as h', 'h.receiptnbr', '=', 'd.receiptnbr')
            ->where('h.receipttype', 'RR')
            ->selectRaw('h.ref_receiptnbr as ref_receiptnbr, SUM(COALESCE(d.qty_return,0)) as sum_returned')
            ->groupBy('h.ref_receiptnbr');
    }
    ...
    public function index()
    {
        ...
        $returnjobs = TrReceipt::query()
            ...
            ->leftJoinSub($returnAgg, 'rr', function($join){
                $join->on('rr.ref_receiptnbr', '=', 'tr_receipt.receiptnbr');
            })
            ->whereRaw('COALESCE(tr_receipt.totalqty_received,0) > COALESCE(rr.sum_returned,0)')
            ->count();
    ...
    public function json(Request $req)
    {
        ...
        if ($scope === 'receiptjobs') {
            ...
            // ✅ khusus returnjobs: PR completed yang masih ada sisa return
            if ($scope === 'returnjobs') {
                $returnAgg = $this->returnAggSubquery();

                $base->where('tr_receipt.status', 'C')
                    ->where('tr_receipt.receipttype', 'PR')
                    ->leftJoinSub($returnAgg, 'rr', function($join){
                        $join->on('rr.ref_receiptnbr', '=', 'tr_receipt.receiptnbr');
                    })
                    ->addSelect([
                        DB::raw('COALESCE(rr.sum_returned,0) as sum_returned'),
                        DB::raw('(COALESCE(tr_receipt.totalqty_received,0) - COALESCE(rr.sum_returned,0)) as remaining_return'),
                    ])
                    ->whereRaw('COALESCE(tr_receipt.totalqty_received,0) > COALESCE(rr.sum_returned,0)');
            }
        ...
        $rows->transform(function($r) use ($scope, $poIdMap) {
            ...
            switch ($st) {
                case 'P': $statusText='Pending';   $statusClass='bg-yellow-100 text-yellow-700 border-yellow-200'; break;
                case 'A': $statusText='Approved';  $statusClass='bg-green-100 text-green-700 border-green-200'; break;
                case 'R': $statusText='Rejected';  $statusClass='bg-red-100 text-red-700 border-red-200'; break;
                case 'C': $statusText='Completed'; $statusClass='bg-emerald-100 text-emerald-700 border-emerald-200'; break;
                case 'D': $statusText='Revise';    $statusClass='bg-blue-100 text-blue-700 border-blue-200'; break;
                case 'X': $statusText='Canceled';  $statusClass='bg-gray-200 text-gray-700 border-gray-300'; break;
                default:  $statusText = $st !== '' ? $st : 'Unknown'; break;
            }
