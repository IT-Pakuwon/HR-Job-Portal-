...
class StrukturOrgController extends Controller
{
    ...
    public function createSto(Request $request)
    {
        ...
        return view('pages.stos.createstos', compact('companies','departements','joblevel','usercpny','usercpny2','userdept','userdept2','sto','users','departments'));
    }
    ...
    public function jsonOrg()
    {
        ...
        foreach ($filtered as $dept) {
            ...
            $data[] = [
                ...
                'parent_id_addition' => $dept->parent_id_addition ? (string)$dept->parent_id_addition : null,
                'name' => $dept->departement_name,
                'position' => 'Department',
                'members' => $memberList->toArray(),
                'image' => 'https://cdn-icons-png.flaticon.com/512/149/149071.png',
        ...
        // 🔗 Ambil koneksi tambahan dari parent_id_addition
        $connections = $filtered->filter(function ($dept) {
            return $dept->parent_id_addition && $dept->parent_id_addition != $dept->parent_id;
        })->map(function ($dept) {
            return [
                'from' => (string) $dept->departement_id,
                'to' => (string) $dept->parent_id_addition,
    ...
    public function jsonOrgByDept($deptname)
    {
        ...
        // 🔗 Koneksi tambahan dari parent_id_addition
        $connections = $filtered->filter(function ($dept) {
            return $dept->parent_id_addition && $dept->parent_id_addition != $dept->parent_id;
        })->map(function ($dept) {
            return [
    ...
    public function getJobProfile($id)
    {
        ...
        $employee = StoEmployee::where('id', $id)           
            ->first();
        // dd($employee);
        $profiles = StoJobProfile::where('job_level', $employee->employee_level)
            ->where('departement_id', $employee->departement_id)
            ->get();
        $spec = StoJobSpec::where('job_level', $employee->employee_level)
            ->orderby('id','Desc')
            ->first();
