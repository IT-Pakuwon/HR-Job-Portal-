...
use App\Models\ViewStagingGRNReturn;
use App\Models\StagingIfcaPoGrnReturn;
...
class IFCAAPIGRNReturnController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $srcQuery = ViewStagingGRNReturn::query()
            ->select([
                'cpny_id',
                'return_no',
                DB::raw('MIN(return_date) as return_date'),
                DB::raw('MIN(grn_no) as grn_no'),
                DB::raw('MIN(department_id) as department_id'),
                DB::raw('MIN(business_unit_id) as business_unit_id'),
            ])
            ->whereBetween('return_date', [$fromDt, $toDt]);
        ...
        $srcRows = $srcQuery
            ->groupBy('cpny_id', 'return_no')
            ->orderByDesc(DB::raw('MIN(return_date)'))
        ...
        $keys = $srcRows
            ->map(fn($r) => (string)$r->cpny_id . '||' . (string)$r->return_no)
            ->values()
            ->all();
        ...
        $stagingAgg = StagingIfcaPoGrnReturn::query()
            ->select([
                'cpny_id',
                'return_no',
                DB::raw('COUNT(*) as cnt'),
            ...
            ->whereIn(DB::raw("(cpny_id || '||' || return_no)"), $keys)
            ->groupBy('cpny_id', 'return_no')
            ->get()
            ->keyBy(fn($r) => (string)$r->cpny_id . '||' . (string)$r->return_no);

        $logRows = TrIntegrationLog::query()
            ->where('integration_id', 'IFCA')
            ->where('setting_id', 'api.Return.url')
            ->whereIn('refnbr', $keys)
            ->orderByDesc('id')
        ...
        $rows = $srcRows->map(function ($r) use ($stagingAgg, $logMap, $businessUnitMap) {
            ...
            return [
                ...
                'return_no'           => $ret,
                'return_date'         => $r->return_date ? Carbon::parse($r->return_date)->format('Y-m-d') : '',
    ...
    public function process(Request $request)
    {
        ...
        $srcHeaders = ViewStagingGRNReturn::query()
            ->select([
                'cpny_id',
                'return_no',
                DB::raw('MIN(business_unit_id) as business_unit_id'),
        ...
        foreach ($srcHeaders as $hdr) {
            $key   = (string)$hdr->cpny_id . '||' . (string)$hdr->return_no;
        ...
        $stMap = StagingIfcaPoGrnReturn::query()
            ->select([
                'cpny_id',
                'return_no',
                DB::raw('COUNT(*) as cnt'),
        ...
        $stagingConn = (new StagingIfcaPoGrnReturn)->getConnectionName();
        DB::connection($stagingConn)->beginTransaction();

        try {
            foreach ($pairs as $p) {
                $cpny = (string)$p['cpny_id'];
                $ret  = (string)$p['return_no'];
                $key  = (string)$p['key'];
                ...
                $lines = ViewStagingGRNReturn::query()
                    ->where('cpny_id', $cpny)
                    ->where('return_no', $ret)
                    ->orderBy('order_line')
                    ->get();
                ...
                foreach ($lines as $ln) {
                    ...
                    StagingIfcaPoGrnReturn::create([
                        'cpny_id'      => (string)$ln->cpny_id,
                        'entity_cd'    => $this->s($entityCd, 20),

                        'return_no'       => (string)$ln->return_no,
                        ...
                        'return_descs' => (string)($ln->return_descs ?? ''),
                        ...
                        'return_qty'      => (float)($ln->return_qty ?? 0),
        ...
        foreach ($pairs as $p) {
            ...
            $cntAll = StagingIfcaPoGrnReturn::query()
                ->where('cpny_id', $cpny)
                ->where('return_no', $ret)
                ->count();

            if ($cntAll > 0) {
                $cntC = StagingIfcaPoGrnReturn::query()
                    ->where('cpny_id', $cpny)
                    ->where('return_no', $ret)
                    ->where('status', 'C')
                    ->count();
            ...
            $lines = StagingIfcaPoGrnReturn::query()
                ->where('cpny_id', $cpny)
                ->where('return_no', $ret)
                ->where('status', 'P')
                ->orderBy('order_line')
        ...
        foreach ($sendPairs as $p) {
            ...
            $res = $this->sendPoGrnReturnAPI($lines, $token, $username, $key);

            if (!empty($res['ok'])) {
                StagingIfcaPoGrnReturn::query()
                    ->where('cpny_id', $cpny)
                    ->where('return_no', $ret)
                    ->where('status', 'P')
                    ->update([
    ...
    private function sendPoGrnReturnAPI($lines, string $token, string $usernameForLog, string $refKey): array
    {
        $url = $this->buildUrl('api.Return.url');
        if ($url === '') {
            throw new \RuntimeException('Setting api.Return.url kosong');
        }

        $settingName = $this->getSettingName('api.Return.url', 'IFCA PO GRN Return');
        ...
        $payload = $lines->map(function ($r) {
            return [
                ...
                'return_descs' => (string)$r->return_descs,
                'grn_no'       => (string)$r->grn_no,
