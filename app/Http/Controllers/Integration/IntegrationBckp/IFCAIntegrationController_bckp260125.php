...
class IFCAIntegrationController extends Controller
{
    ...
    public function nonStockList(Request $request)
    {
        ...
        // ambil log terakhir per refnbr untuk endpoint NonStock
        // ============================
        ...
        // map: refnbr => ['message'=>..., 'last_update'=>...]
        // ambil log terbaru karena order desc
        $logMap = [];
        foreach ($logRows as $lg) {
            $ref = (string) $lg->refnbr;
            if (isset($logMap[$ref])) continue; // sudah terisi (latest)
            ...
            $logMap[$ref] = [
                'message' => $msg,
                // format bebas; ini aman untuk FE (ISO-like)
        ...
        $rows = $invRows->map(function ($r) use ($stagingMap, $logMap) {
            ...
            // kalau H: kosongkan response & last_update
